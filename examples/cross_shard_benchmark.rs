...
use shardx::transaction::{CrossShardManager, CrossShardBenchmarker, OptimizerConfig};
...
async fn main() -> Result<(), Error> {
    ...
    let matches = App::new("ShardX Cross-Shard Transaction Benchmark")
        ...
        .arg(Arg::with_name("batch-size")
            ...
            .default_value("50"))
        .arg(Arg::with_name("batch-interval")
            .long("batch-interval")
            .value_name("MS")
    ...
    // 引数を取得
    let transaction_count = matches.value_of("transactions")
        .unwrap()
        .parse::<usize>()
        .unwrap_or(1000);
    ...
    let timeout_sec = matches.value_of("timeout")
        .unwrap()
        .parse::<u64>()
        .unwrap_or(60);
    ...
    let batch_interval_ms = matches.value_of("batch-interval")
        .unwrap()
        .parse::<u64>()
        .unwrap_or(100);
    ...
    let cross_shard_manager = Arc::new(CrossShardManager::new(shard_manager.clone(), network_tx.clone()));
    ...
    if compare {
        ...
        let (unoptimized, optimized) = benchmarker.run_comparison_benchmark(
            transaction_count,
            concurrency,
            timeout_sec,
        ).await?;
        ...
        // 通常のベンチマークを実行
        info!("Running benchmark with {} transactions, concurrency: {}, timeout: {}s",
...
fn print_benchmark_results(result: &shardx::transaction::BenchmarkResult) {
    ...
    table.add_row(row!["Successful Transactions", result.successful_transactions]);
    table.add_row(row!["Failed Transactions", result.failed_transactions]);
    table.add_row(row!["Success Rate", format!("{:.2}%", (result.successful_transactions as f64 / result.transaction_count as f64) * 100.0)]);
    table.add_row(row!["Total Time", format!("{} ms", result.total_time_ms)]);
    table.add_row(row!["Avg Transaction Time", format!("{:.2} ms", result.avg_transaction_time_ms)]);
    table.add_row(row!["Min Transaction Time", format!("{} ms", result.min_transaction_time_ms)]);
    table.add_row(row!["Max Transaction Time", format!("{} ms", result.max_transaction_time_ms)]);
    table.add_row(row!["Transactions Per Second", format!("{:.2}", result.transactions_per_second)]);
...
fn print_comparison_results(
    ...
    let tps_improvement = (optimized.transactions_per_second - unoptimized.transactions_per_second)
        / unoptimized.transactions_per_second * 100.0;
    
    // レイテンシ改善率を計算
    let latency_improvement = (unoptimized.avg_transaction_time_ms - optimized.avg_transaction_time_ms)
        / unoptimized.avg_transaction_time_ms * 100.0;
    
    table.add_row(row!["Metric", "Without Optimization", "With Optimization", "Improvement"]);
    ...
    table.add_row(row![
        ...
        format!("{:.2}%", (optimized.successful_transactions as f64 - unoptimized.successful_transactions as f64) / unoptimized.successful_transactions as f64 * 100.0)
    ]);
    
    table.add_row(row![
        "Success Rate",
        format!("{:.2}%", (unoptimized.successful_transactions as f64 / unoptimized.transaction_count as f64) * 100.0),
        format!("{:.2}%", (optimized.successful_transactions as f64 / optimized.transaction_count as f64) * 100.0),
        format!("{:.2}%", (optimized.successful_transactions as f64 / optimized.transaction_count as f64) * 100.0 - (unoptimized.successful_transactions as f64 / unoptimized.transaction_count as f64) * 100.0)
    ]);
    ...
    table.add_row(row![
        ...
        format!("{:.2}%", (unoptimized.total_time_ms as f64 - optimized.total_time_ms as f64) / unoptimized.total_time_ms as f64 * 100.0)
    ]);
