...
async fn main() -> Result<(), Error> {
    ...
    println!(
        "Running async runtime benchmark with {} tasks, {} workers",
        task_count, worker_count
    );
    println!(
        "Dependencies: {}, Priorities: {}",
        use_dependencies, use_priorities
    );
    ...
    for i in 0..task_count {
        ...
        // タスクを作成（単純な計算タスク）
        let task_id = async_runtime::spawn_with_priority(
            async move {
                // 単純な計算を実行
                let mut sum = 0;
                for j in 0..1000 {
    ...
    table.add_row(row![
        "Average Task Time",
        format!("{:.2} µs", elapsed_ms * 1000.0 / task_count as f64)
    ]);
