use crate::crypto::zk::{Bulletproof, BulletproofProof};
...
impl BlindingFactor {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, Error> {
        let scalar = Scalar::from_canonical_bytes(bytes.try_into().unwrap_or([0u8; 32]))
            .ok_or_else(|| {
                Error::DeserializationError("Invalid blinding factor format".to_string())
            })?;
...
impl ConfidentialAmount {
    ...
    fn compute_commitment(&mut self) -> Result<(), Error> {
        ...
        // コミットメントを計算
        let commitment =
            &amount_scalar * &RISTRETTO_BASEPOINT_TABLE + &self.blinding.scalar * &h_point;
            ...
            None => {
                ...
                match &clone.commitment {
                    ...
                    None => Err(Error::CryptoError(
                        "Failed to compute commitment".to_string(),
                    )),
                }
            }
        }
...
impl ConfidentialTransaction {
    ...
    fn sign(&mut self, private_key: &[u8]) -> Result<(), Error> {
        ...
        // 秘密鍵をスカラーに変換
        let private_scalar = Scalar::from_canonical_bytes(
            private_key.try_into().unwrap_or([0u8; 32]),
        )
        ...
        let e = Scalar::from_bytes_mod_order_wide(
            &<[u8; 64]>::try_from(&e_hash[..]).unwrap_or([0u8; 64]),
        );
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, Error> {
        ...
        if pos + 2 > bytes.len() {
            return Err(Error::DeserializationError(
                "Invalid recipient data".to_string(),
            ));
        }
        ...
        if pos + recipient_len > bytes.len() {
            return Err(Error::DeserializationError(
                "Invalid recipient length".to_string(),
            ));
        }
        ...
        if pos + 32 > bytes.len() {
            return Err(Error::DeserializationError(
                "Invalid commitment data".to_string(),
            ));
        }
        ...
        if pos + 32 > bytes.len() {
            return Err(Error::DeserializationError(
                "Invalid blinding data".to_string(),
            ));
        }
        ...
        if pos + 4 > bytes.len() {
            return Err(Error::DeserializationError(
                "Invalid range proof length data".to_string(),
            ));
        }
        ...
        if pos + range_proof_len > bytes.len() {
            return Err(Error::DeserializationError(
                "Invalid range proof data".to_string(),
            ));
        }
        ...
        let amount =
            ConfidentialAmount {
                amount: 0,
                blinding,
                commitment: Some(CompressedRistretto::from_slice(commitment_bytes).map_err(
                    |_| Error::DeserializationError("Invalid commitment format".to_string()),
                )?),
            };
...
mod tests {
    ...
    fn test_confidential_transaction() {
        ...
        let recipient_keypair =
            curve25519_dalek::ristretto::RistrettoPoint::random(&mut thread_rng());
        ...
        let range_proof = BulletproofProof {
            inner: bulletproofs::RangeProof::new(
                vec![amount],
                vec![64],
                vec![blinding.scalar],
                None,
