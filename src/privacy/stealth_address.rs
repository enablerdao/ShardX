    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, Error> {
        if bytes.len() != 96 {
            return Err(Error::DeserializationError(
                "Invalid stealth address length".to_string(),
            ));
        }
        ...
        let one_time_pubkey = CompressedRistretto::from_slice(&bytes[32..64]).map_err(|_| {
            Error::DeserializationError("Invalid one-time public key format".to_string())
        })?;
...
impl StealthAddressGenerator {
    ...
    pub fn generate_address(&self, public_key_bytes: &[u8]) -> Result<StealthAddress, Error> {
        ...
        // 公開鍵をポイントに変換
        let public_key_point = public_key.decompress().ok_or_else(|| {
            Error::DeserializationError("Failed to decompress public key".to_string())
        })?;
    ...
    pub fn recover_key_pair(
        ...
        // 秘密鍵をデシリアライズ
        let private_key =
            Scalar::from_canonical_bytes(private_key_bytes.try_into().unwrap_or([0u8; 32]))
                .ok_or_else(|| {
                    Error::DeserializationError("Invalid private key format".to_string())
                })?;
        ...
        // ワンタイム公開鍵をポイントに変換
        let one_time_pubkey_point =
            stealth_address
                .one_time_pubkey
                .decompress()
                .ok_or_else(|| {
                    Error::DeserializationError(
                        "Failed to decompress one-time public key".to_string(),
                    )
                })?;
        ...
        if calculated_hash != stealth_address.shared_secret_hash {
            return Err(Error::AuthenticationError(
                "Shared secret hash mismatch".to_string(),
            ));
        }
        ...
        let one_time_privkey = Scalar::from_bytes_mod_order_wide(
            &<[u8; 64]>::try_from(&one_time_privkey_bytes[..]).unwrap_or([0u8; 64]),
        );
        ...
        if calculated_one_time_pubkey.compress() != stealth_address.one_time_pubkey {
            return Err(Error::AuthenticationError(
                "One-time public key mismatch".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_stealth_address_generation_and_recovery() {
        ...
        // キーペアを復元
        let recovered_key_pair = generator
            .recover_key_pair(&stealth_address, &private_key_bytes)
            .unwrap();
