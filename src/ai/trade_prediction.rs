...
impl PricePredictionModel {
    ...
    pub fn predict_price(&mut self, period: PredictionPeriod) -> Result<PricePrediction, Error> {
        ...
        if self.price_history.len() < 10 {
            return Err(Error::InternalError(
                "Insufficient price history for prediction".to_string(),
            ));
        }
                ...
                let tensor = BUFFER.with(|buffer| {
                    ...
                    input_tensor.with(|tensor_cell| {
                        ...
                        if let Some(tensor) = tensor_opt.as_mut() {
                            ...
                            // 新しいテンソルを作成
                            let new_tensor = tract_ndarray::Array::from_shape_vec(
                                (1, features.len()),
                                buffer.clone(),
                            )
                            .map_err(|e| {
            ...
            let inference_thread = std::thread::spawn(move || {
                ...
                if let Ok(res) = inference_result {
                    ...
                    *result = Some(Ok(res));
                    ...
                    let mut result = result_clone.lock().unwrap();
                    *result = Some(Err(Error::InternalError(format!(
            ...
            if inference_thread.join().is_ok() {
                ...
                if let Some(res) = &*result_guard {
                    match res {
                        Ok(output) => {
                            // 結果を取得
                            let output_view = output[0].to_array_view::<f32>().map_err(|e| {
                                Error::InternalError(format!("Failed to get output: {}", e))
                            })?;
    ...
    fn simple_price_prediction(&self, period: PredictionPeriod) -> (f64, f64, f64) {
        ...
        let std_dev = if change_rates.len() < 2 {
            ...
            let variance = change_rates
                .iter()
                .map(|rate| (*rate - mean).powi(2))
                .sum::<f64>()
                / (change_rates.len() - 1) as f64;
            variance.sqrt()
        };
...
impl TradePredictionManager {
    ...
    pub fn load_model(&self, pair: &TradingPair, model_path: &str) -> Result<(), Error> {
        ...
        let mut models = self.models.lock().unwrap();
        let model = models.get_mut(&pair_str).ok_or_else(|| {
            Error::InternalError(format!("Model for pair {} not found", pair_str))
        })?;
    ...
    pub fn get_last_prediction(&self, pair: &TradingPair) -> Option<PricePrediction> {
        ...
        models
            .get(&pair_str)
            .and_then(|model| model.get_last_prediction())
    }
    ...
    pub fn evaluate_prediction_accuracy(&self, pair: &TradingPair) -> Option<f64> {
        ...
        models
            .get(&pair_str)
            .and_then(|model| model.evaluate_prediction_accuracy())
    }
    ...
    pub fn get_all_predictions(
        &self,
        period: PredictionPeriod,
    ) -> HashMap<String, Result<PricePrediction, Error>> {
...
mod tests {
    ...
    fn test_trade_prediction_manager() {
        ...
        let prediction = manager
            .predict_price(&pair, PredictionPeriod::Hour)
            .unwrap();
