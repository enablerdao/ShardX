...
impl PredictionModel for LinearRegressionModel {
    ...
    fn load(&mut self, path: &str) -> Result<(), Error> {
        ...
        // モデルタイプを確認
        let model_type = model_json["model_type"]
            .as_str()
        ...
        self.intercept = model_json["intercept"]
            .as_f64()
        ...
        self.hyperparameters = serde_json::from_value(model_json["hyperparameters"].clone())
            .map_err(|e| {
                Error::ParseError(format!("ハイパーパラメータの解析に失敗しました: {}", e))
            })?;

        self.trained = model_json["trained"]
            .as_bool()
...
impl PredictionModel for MovingAverageModel {
    ...
    fn load(&mut self, path: &str) -> Result<(), Error> {
        ...
        // パラメータを読み込み
        self.window_size = model_json["window_size"]
            .as_u64()
            .ok_or_else(|| Error::ParseError("ウィンドウサイズが見つかりません".to_string()))?
            as usize;
...
impl PredictionModel for EnsembleModel {
    ...
    fn load(&mut self, path: &str) -> Result<(), Error> {
        ...
        // パラメータを読み込み
        let model_count = model_json["model_count"]
            .as_u64()
            .ok_or_else(|| Error::ParseError("モデル数が見つかりません".to_string()))?
            as usize;
        ...
        for i in 0..model_count {
            ...
            let model_json: serde_json::Value = serde_json::from_str(&model_data).map_err(|e| {
                Error::ParseError(format!("モデルデータの解析に失敗しました: {}", e))
            })?;

            let sub_model_type = model_json["model_type"]
                .as_str()
                .ok_or_else(|| Error::ParseError("モデルタイプが見つかりません".to_string()))?;
            ...
            let mut model: Box<dyn PredictionModel> = match sub_model_type {
                ...
                "MovingAverage" => {
                    let window_size = model_json["window_size"].as_u64().ok_or_else(|| {
                        Error::ParseError("ウィンドウサイズが見つかりません".to_string())
                    })? as usize;
...
impl MarketPredictionService {
    ...
    pub fn update_prediction(
        ...
        if !percentage_errors.is_empty() {
            prediction.mean_absolute_percentage_error =
