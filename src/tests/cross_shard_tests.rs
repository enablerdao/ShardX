use crate::cross_shard::{CrossShardCoordinator, CrossShardMessage, CrossShardMessageType, CrossShardTxStatus};
...
async fn test_cross_shard_transaction_success() {
    ...
    let tx_id = coordinators[0].start_transaction(transaction).await.unwrap();
...
async fn test_cross_shard_transaction_abort() {
    ...
    for i in 1..coordinators.len() {
        ...
        if details.is_ok() {
            let details = details.unwrap();
            assert!(details.status == CrossShardTxStatus::Aborted || 
