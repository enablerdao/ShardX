use crate::multisig::{MultisigManager, MultisigWallet, MultisigTransaction, Operation, TransactionData};
...
fn test_multisig_transaction_creation_and_signing() {
    ...
    let signed_tx2 = multisig_manager.sign_transaction(
        &tx.id,
        "signer2",
        signature2.clone(),
    ).unwrap();
...
fn test_multisig_transaction_rejection() {
    ...
    let signature = vec![1, 2, 3]; // ダミー署名
    let result = multisig_manager.sign_transaction(
        &tx.id,
        "owner1",
        signature,
    );
...
fn test_multisig_wallet_operations() {
    ...
    // 署名を追加
    multisig_manager.sign_transaction(
        &tx.id,
        "owner1",
        vec![1, 2, 3],
    ).unwrap();
    ...
    // 必要署名数変更のトランザクションを作成
    let tx_data = TransactionData {
        operation: Operation::ChangeRequiredSignatures {
            required: 3,
        },
...
fn test_get_wallets_by_account() {
    ...
    // 複数のウォレットを作成
    multisig_manager.create_wallet(
        "Wallet 1".to_string(),
        "owner1",
        vec!["owner1".to_string(), "signer2".to_string()],
        2,
