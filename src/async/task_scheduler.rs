...
impl TaskScheduler {
        ...
        tokio::spawn(async move {
            while let Some(task) = task_receiver.recv().await {
                ...
                let can_process = {
                    ...
                    if all_deps_completed {
                        ...
                        for dep_id in &task.dependencies {
                            queue.dependency_map.entry(dep_id.clone())
                ...
                if can_process {
                    ...
                    let _ = executor_clone.spawn(async move {
                        // トランザクションを処理
                        ...
                        // 統計情報を更新
                        {
                            ...
                            stats.avg_processing_time = stats.total_processing_time / stats.processed_tasks as u32;
    ...
    /// トランザクションをスケジュール
    pub async fn schedule_transaction(&self, tx: Transaction, priority: Option<TaskPriority>) -> Result<(), Error> {
        // 依存関係を抽出
    ...
    /// 複数のトランザクションをスケジュール
    pub async fn schedule_batch(&self, txs: Vec<Transaction>, priority: Option<TaskPriority>) -> Result<(), Error> {
        for tx in txs {
...
mod tests {
    ...
    async fn test_schedule_transaction() {
        ...
        // トランザクションをスケジュール
        let result = scheduler.schedule_transaction(tx, Some(TaskPriority::Normal)).await;
    ...
    async fn test_schedule_batch() {
        ...
        // トランザクションをバッチでスケジュール
        let result = scheduler.schedule_batch(txs, Some(TaskPriority::High)).await;
    ...
    async fn test_dependency_resolution() {
        ...
        // tx2を先にスケジュール
        scheduler.schedule_transaction(tx2, Some(TaskPriority::Normal)).await.unwrap();
        
        // tx1をスケジュール
        scheduler.schedule_transaction(tx1, Some(TaskPriority::Normal)).await.unwrap();
