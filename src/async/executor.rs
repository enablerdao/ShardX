...
impl PriorityAsyncExecutor {
    ...
    pub fn new(max_concurrency: Option<usize>) -> Result<Self, Error> {
        ...
        let low_concurrency = max_concurrency - critical_concurrency - high_concurrency - normal_concurrency;
...
mod tests {
    ...
    fn test_priority_executor() {
        ...
        // すべてのタスクが成功したことを確認
        for result in low_results.iter().chain(normal_results.iter())
            .chain(high_results.iter()).chain(critical_results.iter()) {
            assert!(result.is_ok());
        }
