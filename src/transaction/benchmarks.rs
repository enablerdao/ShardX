...
use crate::transaction::{Transaction, TransactionStatus, CrossShardManager, CrossShardOptimizer, OptimizerConfig};
...
impl CrossShardBenchmarker {
    ...
    pub fn update_optimizer_config(&self, config: OptimizerConfig) -> Result<(), Error> {
        if let Some(optimizer) = &self.cross_shard_optimizer {
            ...
            Err(Error::ValidationError("Optimizer is not enabled".to_string()))
        }
    }
    ...
    async fn generate_test_transactions(&self, count: usize) -> Result<Vec<Transaction>, Error> {
        ...
        if shards.len() < 2 {
            return Err(Error::ValidationError("At least 2 active shards are required for cross-shard benchmark".to_string()));
        }
    ...
    pub async fn run_comparison_benchmark(
        ...
        let unoptimized_result = unoptimized_benchmarker.run_benchmark(
            transaction_count,
            concurrency,
            timeout_sec,
        ).await?;
        ...
        let optimized_result = optimized_benchmarker.run_benchmark(
            transaction_count,
            concurrency,
            timeout_sec,
        ).await?;
        ...
        // 結果を比較
        let improvement = (optimized_result.transactions_per_second - unoptimized_result.transactions_per_second)
            / unoptimized_result.transactions_per_second * 100.0;
        
        info!("Benchmark comparison: Optimization improved TPS by {:.2}%", improvement);
