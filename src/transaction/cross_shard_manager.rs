...
impl CrossShardManager {
    /// 新しいクロスシャードマネージャーを作成
    pub fn new(shard_manager: Arc<ShardManager>, network_tx: mpsc::Sender<NetworkMessage>) -> Self {
        Self {
            shard_manager,
    ...
    /// 子トランザクションを作成
    async fn create_child_transactions(
        &self,
        parent: &Transaction,
    ...
    async fn start_prepare_phase(&self, cross_tx: &CrossShardTransaction) -> Result<(), Error> {
        info!(
            "Starting prepare phase for cross-shard transaction: {}",
            cross_tx.id
        );
        ...
        tokio::spawn(async move {
            ...
            if let Some(tx) = pending.get(&tx_id) {
                if tx.state == CrossShardTransactionState::Preparing {
                    warn!(
                        "Cross-shard transaction timed out in prepare phase: {}",
                        tx_id
                    );
    ...
    /// 準備完了通知を処理
    pub async fn handle_prepare_ack(&self, tx_id: String, shard_id: ShardId) -> Result<(), Error> {
        let mut pending = self.pending_transactions.lock().unwrap();
        ...
        if let Some(tx) = pending.get_mut(&tx_id) {
            if tx.state == CrossShardTransactionState::Preparing {
                ...
                if tx.prepared_shards == tx.involved_shards {
                    info!(
                        "All shards prepared for transaction {}. Starting commit phase.",
                        tx_id
                    );
    ...
    async fn start_commit_phase(&self, cross_tx: &CrossShardTransaction) -> Result<(), Error> {
        info!(
            "Starting commit phase for cross-shard transaction: {}",
            cross_tx.id
        );
    ...
    /// コミット完了通知を処理
    pub async fn handle_commit_ack(&self, tx_id: String, shard_id: ShardId) -> Result<(), Error> {
        let mut pending = self.pending_transactions.lock().unwrap();
        ...
        if let Some(tx) = pending.get_mut(&tx_id) {
            if tx.state == CrossShardTransactionState::Committing {
                ...
                if tx.committed_shards == tx.involved_shards {
                    info!(
                        "All shards committed for transaction {}. Transaction completed.",
                        tx_id
                    );
                    ...
                    tokio::spawn(async move {
                        ...
                        debug!(
                            "Removed completed cross-shard transaction from pending: {}",
                            tx_id_clone
                        );
                    });
                }
    ...
    /// 中止通知を処理
    pub async fn handle_abort(&self, tx_id: String, shard_id: ShardId) -> Result<(), Error> {
        let mut pending = self.pending_transactions.lock().unwrap();
