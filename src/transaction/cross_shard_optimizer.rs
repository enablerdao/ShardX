...
impl CrossShardOptimizer {
        ...
        tokio::spawn(async move {
            loop {
                ...
                for (shard_id, batch) in batches {
                    ...
                    tokio::spawn(async move {
                        debug!(
                            "Processing batch of {} transactions for shard {}",
                            batch.len(),
                            shard_id
                        );
                        ...
                        for tx in batch {
                            ...
                            let handle = tokio::spawn(async move {
                                let result =
                                    cross_shard_manager.start_transaction(tx.clone()).await;
    ...
    pub fn get_optimal_path(&self, from_shard: &ShardId, to_shard: &ShardId) -> Vec<ShardId> {
        ...
        routing_table
            .get(&(from_shard.clone(), to_shard.clone()))
            .cloned()
...
fn calculate_shortest_path(
    ...
    // 距離テーブルを初期化
    let mut distances: HashMap<ShardId, u64> = shard_ids
        .iter()
        .map(|id| (id.clone(), if id == from_shard { 0 } else { u64::MAX }))
        .collect();

    // 前のノードを追跡
    let mut previous: HashMap<ShardId, Option<ShardId>> =
        shard_ids.iter().map(|id| (id.clone(), None)).collect();
