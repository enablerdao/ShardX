...
use crate::transaction::{Transaction, TransactionStatus, CrossShardManager, CrossShardOptimizer, OptimizerConfig, ParallelProcessor, ProcessorConfig};
...
impl MemoryPool {
    ...
    fn add_transaction(&mut self, transaction: Transaction) -> Result<(), Error> {
        ...
        // アドレス別トランザクションに追加
        let addr_txs = self.address_transactions.entry(from_addr).or_insert_with(HashSet::new);
        addr_txs.insert(tx_id.clone());

        // シャード別トランザクションに追加
        let shard_txs = self.shard_transactions.entry(shard_id).or_insert_with(HashSet::new);
    ...
    fn confirm_transaction(&mut self, tx_id: &str) -> Result<(), Error> {
        if let Some(tx) = self.pending_transactions.remove(tx_id) {
            ...
            Err(Error::NotFoundError(format!("Transaction {} not found in pending pool", tx_id)))
        }
    }
    ...
    fn reject_transaction(&mut self, tx_id: &str, reason: &str) -> Result<(), Error> {
        if let Some(tx) = self.pending_transactions.remove(tx_id) {
            self.rejected_transactions.insert(tx_id.to_string(), (tx, reason.to_string()));
            Ok(())
        } else {
            Err(Error::NotFoundError(format!("Transaction {} not found in pending pool", tx_id)))
        }
    ...
    fn cleanup(&mut self, max_age_sec: u64) -> usize {
        ...
        // 古い確認済みトランザクションを削除
        let confirmed_to_remove: Vec<String> = self.confirmed_transactions.iter()
        ...
        // 古い拒否されたトランザクションを削除
        let rejected_to_remove: Vec<String> = self.rejected_transactions.iter()
    ...
    fn get_stats(&self) -> MemoryPoolStats {
        MemoryPoolStats {
            ...
            total_count: self.pending_transactions.len() + self.confirmed_transactions.len() + self.rejected_transactions.len(),
...
impl HighThroughputEngine {
    ...
    pub fn start(&self) -> Result<(), Error> {
        {
            ...
            if *running {
                return Err(Error::ValidationError("Engine is already running".to_string()));
            }
            *running = true;
        }
        
        info!("Starting HighThroughputEngine with max throughput: {} TPS", self.config.max_throughput);
            ...
            while *running.read().unwrap() {
                ...
                if let Ok(results) = results {
                    ...
                    // 統計を更新
                    {
                        ...
                        // 平均処理時間を更新
                        let total_time = stats.avg_processing_time_ms * (stats.processed_transactions - batch.len() as u64) as f64 + processing_time as f64;
            ...
            while *running.read().unwrap() {
                ...
                if elapsed > 0 {
                    ...
                    // エンジン統計を更新
                    {
                        ...
                        stats_guard.max_throughput = stats_guard.max_throughput.max(stats_guard.current_throughput);
        ...
        tokio::spawn(async move {
            while *running.read().unwrap() {
                ...
                let (current_load, current_throughput, current_batch_size) = {
                    let stats = stats.read().unwrap();
                    (stats.current_load, stats.current_throughput, stats.current_batch_size)
                };
                ...
                } else if current_load < config.low_load_threshold && current_throughput < config.max_throughput as f64 * 0.8 {
                ...
                if new_batch_size != current_batch_size {
                    ...
                    debug!("Adjusted batch size from {} to {} (load: {:.2}, throughput: {:.2} TPS)",
    ...
    pub async fn run_benchmark(
        ...
        info!("Starting benchmark with {} transactions, concurrency: {}, timeout: {}s",
        ...
        info!("Benchmark completed: {} TPS, avg time: {:.2}ms, success rate: {:.2}%",
