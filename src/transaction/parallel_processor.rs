...
impl ParallelProcessor {
    ...
    pub fn start(&self) {
        info!(
            "Starting ParallelProcessor with max parallelism: {}",
    ...
    pub async fn process_transactions(
        ...
        // 結果を格納する配列
        let mut results =
            vec![Err(Error::InternalError("Not processed".to_string())); transactions.len()];
        let tx_indices: HashMap<String, usize> = transactions
            .iter()
            .enumerate()
    ...
    async fn analyze_dependencies(&self, transaction: &Transaction) -> Result<(), Error> {
        ...
        {
            let mut processing_txs = self.processing_txs.lock().unwrap();
            processing_txs.insert(
                transaction.id.clone(),
                TransactionProcessingState::ResolvingDependencies,
            );
        }
    ...
    /// 2. 依存関係の計算を効率化
    /// 3. 不要なクローンを削減
    async fn analyze_batch_dependencies(
        &self,
        ...
        // 依存関係グラフを更新（バッチ更新で最適化）
        {
            ...
            for (tx_id, deps) in &direct_dependencies {
                if !deps.is_empty() {
                    let graph_deps = dependency_graph
                        .entry(tx_id.clone())
                        .or_insert_with(HashSet::new);
    ...
    fn group_by_dependencies(
        ...
        let mut reverse_deps: HashMap<String, HashSet<String>> =
        ...
        while !dependent_txs.is_empty() && iteration_count < max_iterations {
            ...
            if next_group.is_empty() {
                // 循環依存関係を検出して解決
                let mut cycle_breakers =
                    self.detect_cycle_breakers(&remaining, &dependencies, &processed_txs);
    ...
    fn update_stats(&self, tx_id: String, is_success: bool, processing_time: u64) {
        // 統計を更新
        {
            ...
            stats.avg_processing_time_ms =
                (1.0 - alpha) * stats.avg_processing_time_ms + alpha * processing_time as f64;
            ...
            // 現在の並列度を更新
            stats.current_parallelism =
                self.config.max_parallelism - self.semaphore.available_permits();

            // 現在の負荷を更新
            stats.current_load =
                stats.current_parallelism as f64 / self.config.max_parallelism as f64;
        ...
        // 処理状態を更新
        {
            ...
            if is_success {
                ...
                processing_txs.insert(
                    tx_id,
                    TransactionProcessingState::Failed("Transaction failed".to_string()),
                );
            }
        }
    }
            ...
            loop {
                ...
                if avg_load > config.high_load_threshold {
                    ...
                    if avg_throughput > 0.0 && used_permits > 0 {
                        ...
                        let target_permits =
                            (target_throughput / throughput_per_permit).ceil() as usize;
                        ...
                        if additional_permits > 0 {
                            ...
                            info!(
                                "Increased parallelism to {} permits based on throughput analysis",
                                target_permits
                            );
                        }
                    } else {
                        ...
                        // 現在の許可数との差分を追加
                        let additional_permits =
                            new_permits.saturating_sub(max_permits - current_permits);
...
mod tests {
    ...
    async fn test_group_by_dependencies() {
        ...
        // 並列処理器を作成
        let processor =
            ParallelProcessor::new(cross_shard_manager, shard_manager, network_tx, None);
