...
use crate::chart::{DataPoint, TimeFrame, ChartDataAggregator};
...
impl DetailedTransactionAnalyzer {
    ...
    pub fn analyze(&self) -> Result<TransactionAnalysisResult, Error> {
        ...
        let filtered_transactions: Vec<&Transaction> = self.transactions.iter()
            .filter(|tx| {
        ...
        if filtered_transactions.is_empty() {
            return Err(Error::InvalidInput("分析対象期間内にトランザクションがありません".to_string()));
        }
        
        // 基本的な統計情報を計算
        let total_transactions = filtered_transactions.len();
        
        let successful_transactions = filtered_transactions.iter()
            .filter(|tx| tx.status == TransactionStatus::Confirmed)
            .count();
        
        let failed_transactions = filtered_transactions.iter()
            .filter(|tx| tx.status == TransactionStatus::Failed)
            .count();
        
        let pending_transactions = filtered_transactions.iter()
            .filter(|tx| tx.status == TransactionStatus::Pending)
            .count();
        
        // 取引量の統計を計算
        let volumes: Vec<f64> = filtered_transactions.iter()
        ...
        // 手数料の統計を計算
        let fees: Vec<f64> = filtered_transactions.iter()
        ...
        let top_receivers_by_volume = self.calculate_top_receivers_by_volume(&filtered_transactions);
        
        // シャード関連の統計を計算
        let (cross_shard_transactions, intra_shard_transactions) = self.calculate_shard_transaction_counts(&filtered_transactions);
    ...
    fn calculate_transactions_by_time(&self, transactions: &[&Transaction]) -> Vec<DataPoint> {
        ...
        // DataPointに変換
        let mut data_points: Vec<DataPoint> = time_counts.iter()
    ...
    fn calculate_volume_by_time(&self, transactions: &[&Transaction]) -> Vec<DataPoint> {
        ...
        // DataPointに変換
        let mut data_points: Vec<DataPoint> = time_volumes.iter()
    ...
    fn calculate_fees_by_time(&self, transactions: &[&Transaction]) -> Vec<DataPoint> {
        ...
        // DataPointに変換
        let mut data_points: Vec<DataPoint> = time_fees.iter()
    ...
    /// 上位受信者（取引量ベース）を計算
    fn calculate_top_receivers_by_volume(&self, transactions: &[&Transaction]) -> Vec<(String, f64)> {
        // 受信者ごとの取引量を集計
    ...
    /// シャード別トランザクション数を計算
    fn calculate_transactions_by_shard(&self, transactions: &[&Transaction]) -> HashMap<ShardId, usize> {
    ...
    /// 循環取引パターンを検出
    fn detect_circular_transactions(&self, transactions: &[&Transaction]) -> Vec<TransactionPattern> {
        // アドレス間の取引グラフを構築
        ...
        for start_address in graph.keys() {
            ...
            if self.find_cycle(start_address, &graph, &mut visited, &mut path, &mut path_txs, start_address, 0) {
                // 循環が見つかった場合、パターンを作成
                if path.len() >= 3 {
                    ...
                    let transaction_ids: Vec<String> = path_txs.iter().map(|tx| tx.id.clone()).collect();
                    
                    let pattern = TransactionPattern {
                        id: pattern_id,
                        name: format!("循環取引 ({} アドレス)", path.len()),
                        description: format!("{}個のアドレス間で循環する取引パターンが検出されました", path.len()),
                        addresses: path,
                        occurrences: 1,
    ...
    fn detect_split_transactions(&self, transactions: &[&Transaction]) -> Vec<TransactionPattern> {
        ...
        for (sender, txs) in sender_txs {
            ...
            while i < sorted_txs.len() {
                ...
                let group: Vec<&Transaction> = sorted_txs.iter()
                    .filter(|tx| {
                        let tx_time = Utc.timestamp(tx.timestamp, 0);
                ...
                if group.len() >= 3 {
                    let receivers: HashSet<String> = group.iter().map(|tx| tx.receiver.clone()).collect();
                    ...
                    if receivers.len() >= 3 {
                        ...
                        let all_txs_avg = transactions.iter().map(|tx| tx.amount as f64).sum::<f64>() / transactions.len() as f64;
                        ...
                        if average_volume < all_txs_avg * 0.2 {
                            ...
                            let transaction_ids: Vec<String> = group.iter().map(|tx| tx.id.clone()).collect();
    ...
    fn detect_merge_transactions(&self, transactions: &[&Transaction]) -> Vec<TransactionPattern> {
        ...
        for tx in transactions {
            let txs = receiver_txs.entry(tx.receiver.clone()).or_insert_with(Vec::new);
            txs.push(tx);
        }
        ...
        for (receiver, txs) in receiver_txs {
            ...
            while i < sorted_txs.len() {
                ...
                if group.len() >= 3 {
                    let senders: HashSet<String> = group.iter().map(|tx| tx.sender.clone()).collect();
    ...
    /// 大量取引の異常を検出
    fn detect_large_transaction_anomalies(&self, transactions: &[&Transaction]) -> Vec<AnomalyDetectionResult> {
        // 取引量の統計を計算
        ...
        for tx in transactions {
            ...
            if volume > threshold {
                ...
                let anomaly = AnomalyDetectionResult {
                    ...
                    description: format!("通常の{}倍の大量取引が検出されました", (volume / average_volume).round()),
    ...
    /// 異常な取引頻度を検出
    fn detect_frequency_anomalies(&self, transactions: &[&Transaction]) -> Vec<AnomalyDetectionResult> {
        // アドレスごとの取引頻度を計算
        ...
        for (address, frequency) in address_frequencies {
            if frequency as f64 > threshold {
                // 関連するトランザクションを取得
                let related_txs: Vec<&Transaction> = transactions.iter()
                ...
                let transaction_ids: Vec<String> = related_txs.iter().map(|tx| tx.id.clone()).collect();
    ...
    pub fn analyze_address_relationships(&self) -> Vec<AddressRelationship> {
        ...
        for ((sender, receiver), txs) in pair_txs {
            ...
            let max_volume = pair_txs.values().map(|v| v.iter().map(|tx| tx.amount as f64).sum::<f64>()).max_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal)).unwrap_or(1.0);
...
mod tests {
    ...
    fn test_pattern_detection() {
        ...
        // 循環取引パターンをチェック
        let circular_patterns: Vec<&TransactionPattern> = patterns.iter()
        ...
        // 分割取引パターンをチェック
        let split_patterns: Vec<&TransactionPattern> = patterns.iter()
        ...
        // 統合取引パターンをチェック
        let merge_patterns: Vec<&TransactionPattern> = patterns.iter()
    ...
    fn test_anomaly_detection() {
        ...
        // 大量取引の異常をチェック
        let large_tx_anomalies: Vec<&AnomalyDetectionResult> = anomalies.iter()
        ...
        // 異常な取引頻度をチェック
        let frequency_anomalies: Vec<&AnomalyDetectionResult> = anomalies.iter()
            .filter(|a| a.anomaly_type == AnomalyType::AbnormalFrequency)
            .collect();
        
        assert!(!frequency_anomalies.is_empty());
        assert!(frequency_anomalies[0].addresses.contains(&"addr1".to_string()));
    }
