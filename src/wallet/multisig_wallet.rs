...
use crate::wallet::{Wallet, WalletStatus, WalletType};
...
impl MultisigWalletManager {
    ...
    pub fn create_wallet(
        ...
        if owners.is_empty() {
            return Err(Error::InvalidInput(
                "Owners list cannot be empty".to_string(),
            ));
        }
        ...
        // 重複する所有者を削除
        let unique_owners: Vec<PublicKey> = owners
            .clone()
            .into_iter()
            .collect::<HashSet<_>>()
            .into_iter()
    ...
    pub fn create_transaction(
        ...
        wallet
            ...
            .insert(multisig_tx_id.clone(), multisig_tx.clone());
    ...
    pub fn sign_transaction(
        ...
        if let Some(expires_at) = multisig_tx.expires_at {
            if Utc::now() > expires_at {
                ...
                wallet
                    .pending_transactions
                    .insert(transaction_id.to_string(), multisig_tx.clone());
                self.update_wallet(&wallet)?;
        ...
        if !wallet.owners.contains(&public_key) {
            return Err(Error::Unauthorized(format!(
                "Signer is not an owner of the wallet: {}",
                public_key
            )));
        }
    ...
    pub fn execute_transaction(
        ...
        let combined_signature =
            combine_signatures(&multisig_tx.signatures.values().cloned().collect());
        ...
        wallet
            .pending_transactions
            .insert(transaction_id.to_string(), multisig_tx);
        wallet.updated_at = Utc::now();
    ...
    pub fn get_pending_transactions(
        ...
        // 保留中のトランザクションを取得
        let pending_txs: Vec<MultisigTransaction> = wallet
            .pending_transactions
            .values()
            .filter(|tx| {
    ...
    pub fn get_executed_transactions(
        ...
        // 実行済みのトランザクションを取得
        let executed_txs: Vec<MultisigTransaction> = wallet
            .pending_transactions
            .values()
    ...
    pub fn add_owner(
        ...
        if signatures < wallet.required_signatures {
            return Err(Error::Unauthorized(format!(
                ...
                signatures, wallet.required_signatures
            )));
        }
