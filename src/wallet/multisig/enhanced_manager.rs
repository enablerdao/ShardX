...
impl EnhancedMultisigManager {
    ...
    ) -> Result<EnhancedMultisigWallet, Error> {
        if !policy.is_valid() {
    ...
    pub fn create_transaction(
    ...
    ) -> Result<EnhancedMultisigTransaction, Error> {
        // ウォレットを取得
        ...
        // マルチシグトランザクションを作成
        let multisig_tx =
            EnhancedMultisigTransaction::new(wallet_id.clone(), transaction, wallet.policy.clone());
    ...
    pub fn update_transaction(
        &self,
        transaction: EnhancedMultisigTransaction,
    ) -> Result<(), Error> {
        let mut transactions = self.transactions.write().unwrap();
    ...
    pub fn get_wallet_transactions(
        &self,
        wallet_id: &WalletId,
    ) -> Vec<EnhancedMultisigTransaction> {
        let transactions = self.transactions.read().unwrap();
    ...
    pub fn get_wallet_stats(&self, wallet_id: &WalletId) -> Result<WalletStats, Error> {
        ...
        let pending_transactions = transactions
            .iter()
            .filter(|tx| {
                tx.state == MultisigTransactionState::Created
                    || tx.state == MultisigTransactionState::PartiallyApproved
            })
            .count();
        ...
        let rejected_transactions = transactions
            .iter()
            .filter(|tx| tx.state == MultisigTransactionState::Rejected)
            .count();

        let expired_transactions = transactions
            .iter()
            .filter(|tx| tx.state == MultisigTransactionState::Expired)
            .count();

        let total_volume: u64 = transactions
            .iter()
            .filter(|tx| tx.state == MultisigTransactionState::Executed)
        ...
        let avg_approval_time = if executed_transactions > 0 {
            let total_time: i64 = transactions
                .iter()
                .filter(|tx| {
                    tx.state == MultisigTransactionState::Executed && tx.executed_at.is_some()
                })
...
impl EnhancedMultisigFactory {
    ...
    /// 重み付きマルチシグウォレットを作成
    pub fn create_weighted_wallet(
        &self,
        name: String,
        keys: Vec<PublicKey>,
        weights: HashMap<PublicKey, u32>,
        threshold: u32,
    ) -> Result<EnhancedMultisigWallet, Error> {
    ...
    /// 有効期限付きマルチシグウォレットを作成
    pub fn create_time_locked_wallet(
        &self,
        name: String,
...
mod tests {
    ...
    fn test_multisig_manager() {
        ...
        // ウォレットを作成
        let wallet = manager
            .create_wallet("テストウォレット".to_string(), policy)
            .unwrap();
        ...
        // 署名を追加
        let tx1 = manager
            .add_signature(&multisig_tx.id, keypair1.public.clone(), "sig1".to_string())
            .unwrap();
        assert_eq!(tx1.state, MultisigTransactionState::PartiallyApproved);

        let tx2 = manager
            .add_signature(&multisig_tx.id, keypair2.public.clone(), "sig2".to_string())
            .unwrap();
    ...
    fn test_multisig_factory() {
        ...
        let weighted_wallet = factory
            .create_weighted_wallet(
                "重み付きウォレット".to_string(),
                vec![
                    keypair1.public.clone(),
                    keypair2.public.clone(),
        ...
        let limited_wallet = factory
            .create_limited_wallet("制限付きウォレット".to_string(), policy, 500)
            .unwrap();
