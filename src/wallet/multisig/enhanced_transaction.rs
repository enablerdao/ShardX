...
impl EnhancedMultisigTransaction {
    ...
    pub fn new(wallet_id: WalletId, transaction: Transaction, policy: ThresholdPolicy) -> Self {
        let now = Utc::now();
        let id = format!("multisig-{}", hash(&format!("{}-{}-{}", wallet_id, transaction.id, now)));
        
        // 有効期限を設定（デフォルトは24時間）
        let expires_at = policy.expiration.or_else(|| Some(now + chrono::Duration::hours(24)));
    ...
    /// 署名を追加
    pub fn add_signature(&mut self, public_key: PublicKey, signature: Signature) -> Result<(), Error> {
        // 状態をチェック
        if self.state != MultisigTransactionState::Created && self.state != MultisigTransactionState::PartiallyApproved {
        ...
        if let Some(expires_at) = self.expires_at {
            if Utc::now() > expires_at {
                ...
                return Err(Error::Expired("トランザクションの有効期限が切れています".to_string()));
            }
        }
    ...
    pub fn execute(&mut self) -> Result<(), Error> {
        ...
        // 閾値を満たしているかどうかを再確認
        if !self.policy.is_threshold_met(&self.signatures) {
            return Err(Error::Unauthorized("必要な署名数を満たしていません".to_string()));
        }
    ...
    pub fn get_summary(&self) -> String {
        ...
        let signatures = format!("{}/{}", self.signature_count(), self.policy.required_signatures);
...
mod tests {
    ...
    fn test_multisig_transaction() {
        ...
        // マルチシグトランザクションを作成
        let mut multisig_tx = EnhancedMultisigTransaction::new("wallet1".to_string(), transaction, policy);
        ...
        // 署名を追加
        multisig_tx.add_signature(keypair1.public.clone(), "sig1".to_string()).unwrap();
        
        // 状態を確認
        assert_eq!(multisig_tx.state, MultisigTransactionState::PartiallyApproved);
        assert_eq!(multisig_tx.signature_count(), 1);
        assert_eq!(multisig_tx.remaining_signatures(), 1);
        
        // 署名を追加
        multisig_tx.add_signature(keypair2.public.clone(), "sig2".to_string()).unwrap();
    ...
    fn test_metadata() {
        ...
        // メタデータを取得
        assert_eq!(multisig_tx.get_metadata("purpose"), Some(&"給与支払い".to_string()));
        assert_eq!(multisig_tx.get_metadata("department"), Some(&"経理部".to_string()));
