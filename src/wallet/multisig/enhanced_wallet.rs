...
impl EnhancedMultisigWallet {
    ...
    pub fn set_daily_limit(&mut self, limit: u64) {
        ...
        let reset_time = Utc.ymd(tomorrow.year(), tomorrow.month(), tomorrow.day()).and_hms(0, 0, 0);
    ...
    pub fn reset_daily_limit_if_needed(&mut self) {
        if let (Some(limit), Some(reset_time)) = (self.daily_limit, self.limit_reset_time) {
            ...
            if now >= reset_time {
                ...
                // 次の制限リセット時刻を設定
                let tomorrow = now + chrono::Duration::days(1);
                let next_reset = Utc.ymd(tomorrow.year(), tomorrow.month(), tomorrow.day()).and_hms(0, 0, 0);
...
mod tests {
    ...
    fn test_multisig_wallet() {
        ...
        // メタデータを取得
        assert_eq!(wallet.get_metadata("purpose"), Some(&"プロジェクト資金".to_string()));
        assert_eq!(wallet.get_metadata("owner"), Some(&"財務部".to_string()));
        
        // 説明を設定
        wallet.set_description("プロジェクトXの資金管理用ウォレット");
        assert_eq!(wallet.description, Some("プロジェクトXの資金管理用ウォレット".to_string()));
        
        // タグを追加
        wallet.add_tag("プロジェクト");
        wallet.add_tag("財務");
        assert_eq!(wallet.tags, vec!["プロジェクト".to_string(), "財務".to_string()]);
