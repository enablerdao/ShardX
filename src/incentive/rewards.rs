...
impl RewardManager {
    ...
    pub fn record_reward(
        ...
        // 受取人の報酬リストに追加
        let recipient_rewards = self.recipient_rewards
        ...
        info!("Reward recorded: {} tokens to {} ({})", amount, recipient, reward_id);
    ...
    pub fn record_distribution(
        ...
        info!("Reward distribution recorded: {} tokens at block {}", total_amount, block_height);
    ...
    pub fn pay_reward(
        ...
        // 報酬を取得
        let reward = self.rewards.get_mut(reward_id)
    ...
    pub fn get_reward_history(&self, recipient: &str) -> Result<Vec<Reward>, Error> {
        let reward_ids = self.recipient_rewards.get(recipient)
            .cloned()
            .unwrap_or_default();
        
        let rewards: Vec<Reward> = reward_ids.iter()
...
mod tests {
    ...
    fn test_reward_recording() {
        ...
        // 報酬を記録
        let reward_id = manager.record_reward(
            "recipient1",
            100,
    ...
    fn test_max_supply_limit() {
        ...
        // 最大供給量を超える報酬を記録
        let result = manager.record_reward(
            "recipient3",
            200,
            RewardType::LiquidityProvider,
            12347,
