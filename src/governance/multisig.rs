...
impl MultisigWallet {
    ...
    pub fn add_owner(&mut self, owner: Owner) -> Result<(), Error> {
        // 所有者の重複をチェック
        if self.owners.iter().any(|o| o.id == owner.id || o.address == owner.address) {
            return Err(Error::AlreadyExists(format!("Owner already exists: {}", owner.id)));
        }
    ...
    pub fn remove_owner(&mut self, owner_id: &str) -> Result<(), Error> {
        ...
        if self.threshold > total_weight {
            return Err(Error::InvalidState(format!("Threshold ({}) exceeds total weight ({})", self.threshold, total_weight)));
        }
    ...
    pub fn change_threshold(&mut self, threshold: u32) -> Result<(), Error> {
        ...
        if threshold > total_weight {
            return Err(Error::InvalidInput(format!("Threshold ({}) exceeds total weight ({})", threshold, total_weight)));
        }
        
        if threshold == 0 {
            return Err(Error::InvalidInput("Threshold must be greater than 0".to_string()));
        }
    ...
    pub fn create_transaction(
        ...
        if !self.owners.iter().any(|o| o.id == creator) {
            return Err(Error::InvalidInput(format!("Creator is not an owner: {}", creator)));
        }
    ...
    /// トランザクションに署名
    pub fn sign_transaction(&mut self, transaction_id: &str, owner_id: &str, signature: Signature) -> Result<(), Error> {
        // トランザクションを取得
        ...
        if transaction.status != TransactionStatus::Pending {
            return Err(Error::InvalidState(format!("Transaction is not pending: {:?}", transaction.status)));
        }
        
        // 所有者をチェック
        let owner = self.owners.iter().find(|o| o.id == owner_id).ok_or_else(|| {
            Error::NotFound(format!("Owner not found: {}", owner_id))
        })?;
        
        // 署名を検証
        let transaction_data = self.get_transaction_data(transaction);
        let valid = owner.public_key.verify(&transaction_data, &signature)
        ...
        // 署名を追加
        transaction.signatures.insert(owner_id.to_string(), signature);
        transaction.updated_at = Utc::now();
        
        // 署名の重みを計算
        let signed_weight: u32 = transaction.signatures.keys()
    ...
    pub fn execute_transaction(&mut self, transaction_id: &str) -> Result<(), Error> {
        ...
        if transaction.status != TransactionStatus::Executable {
            return Err(Error::InvalidState(format!("Transaction is not executable: {:?}", transaction.status)));
        }
        ...
        // 閾値に達しているかチェック
        if signed_weight < self.threshold {
            return Err(Error::InvalidState(format!("Insufficient signatures: {} < {}", signed_weight, self.threshold)));
        }
        ...
        transaction.transaction_hash = Some(format!("0x{}", hex::encode(hash(&self.get_transaction_data(transaction)))));
    ...
    /// トランザクションをキャンセル
    pub fn cancel_transaction(&mut self, transaction_id: &str, owner_id: &str) -> Result<(), Error> {
        // トランザクションを取得
        let transaction = self.transactions.get_mut(transaction_id).ok_or_else(|| {
            Error::NotFound(format!("Transaction not found: {}", transaction_id))
        })?;
        ...
        // トランザクションのステータスをチェック
        if transaction.status != TransactionStatus::Pending && transaction.status != TransactionStatus::Executable {
            return Err(Error::InvalidState(format!("Transaction cannot be cancelled: {:?}", transaction.status)));
        }
        ...
        if transaction.creator != owner_id && !transaction.signatures.contains_key(owner_id) {
            return Err(Error::InvalidState(format!("Only creator or signers can cancel the transaction")));
        }
    ...
    /// トランザクションの署名を検証
    pub fn verify_transaction_signature(&self, transaction_id: &str, owner_id: &str) -> Result<bool, Error> {
        // トランザクションを取得
        let transaction = self.transactions.get(transaction_id).ok_or_else(|| {
            Error::NotFound(format!("Transaction not found: {}", transaction_id))
        })?;
        ...
        // 署名を検証
        let transaction_data = self.get_transaction_data(transaction);
        owner.public_key.verify(&transaction_data, signature)
            .map_err(|e| Error::CryptoError(format!("Signature verification failed: {}", e)))
    }
    ...
    pub fn change_owner_weight(&mut self, owner_id: &str, weight: u32) -> Result<(), Error> {
        // 所有者を取得
        let owner = self.owners.iter_mut().find(|o| o.id == owner_id).ok_or_else(|| {
            Error::NotFound(format!("Owner not found: {}", owner_id))
        })?;
    ...
    /// 所有者の公開鍵を更新
    pub fn update_owner_public_key(&mut self, owner_id: &str, public_key: PublicKey) -> Result<(), Error> {
        // 所有者を取得
        let owner = self.owners.iter_mut().find(|o| o.id == owner_id).ok_or_else(|| {
            Error::NotFound(format!("Owner not found: {}", owner_id))
        })?;
    ...
    pub fn update_owner_address(&mut self, owner_id: &str, address: String) -> Result<(), Error> {
        // アドレスの重複をチェック
        if self.owners.iter().any(|o| o.id != owner_id && o.address == address) {
            return Err(Error::AlreadyExists(format!("Address already in use: {}", address)));
        }
    ...
    /// ウォレットのメタデータを更新
    pub fn update_metadata(&mut self, metadata: Option<HashMap<String, serde_json::Value>>) -> Result<(), Error> {
