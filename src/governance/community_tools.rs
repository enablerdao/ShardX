...
use crate::governance::{
    DAOMember, DAORole, Proposal, ProposalStatus, ProposalType, Treasury, TreasuryTransaction,
    Vote, VotingPower, DAO,
};
...
impl CommunityToolsManager {
    ...
    pub fn create_task(
        ...
        if !self.config.enable_task_management {
            return Err(Error::InvalidState(
                "Task management is not enabled".to_string(),
            ));
        }
    ...
    pub fn update_task_status(
        ...
        // メタデータを更新
        task.metadata
            .insert("last_updated_by".to_string(), updater.to_string());
    ...
    pub fn create_notification(
        ...
        // 通知を保存
        let recipient_notifications = self
            .notifications
    ...
    pub fn mark_notification_as_read(
        ...
        // 受信者の通知を取得
        let recipient_notifications = self.notifications.get_mut(recipient).ok_or_else(|| {
            Error::NotFound(format!(
                "No notifications found for recipient: {}",
                recipient
            ))
        })?;

        // 通知を検索
        let notification = recipient_notifications
            .iter_mut()
            .find(|n| n.id == notification_id)
            .ok_or_else(|| {
                Error::NotFound(format!("Notification not found: {}", notification_id))
            })?;
    ...
    pub fn get_forum_topics_by_category(
        &self,
        category_id: &str,
    ...
    pub fn get_forum_replies_by_topic(&self, topic_id: &str) -> Result<Vec<&ForumReply>, Error> {
        ...
        // トピックの返信リストを取得
        let replies: Vec<&ForumReply> = self
            .forum_replies
            .values()
    ...
    /// 受信者の通知リストを取得
    pub fn get_notifications_by_recipient(
        &self,
        recipient: &str,
    ...
    pub fn get_analytics_data_by_type_and_period(
        ...
        // タイプと期間の分析データリストを取得
        let data: Vec<&AnalyticsData> = self
            .analytics_data
            .values()
...
mod tests {
    ...
    fn test_forum_functionality() {
        ...
        // 返信を作成
        let reply_id = manager
            .create_forum_reply(&topic_id, "Thanks for the welcome!", "user1", None)
            .unwrap();
    ...
    fn test_task_management() {
        ...
        // タスクステータスを更新
        manager
            .update_task_status(&task_id, TaskStatus::InProgress, "developer1")
            .unwrap();
        ...
        // タスクリストを取得
        let tasks = manager
            .get_tasks_by_status(Some(TaskStatus::InProgress))
            .unwrap();
