...
pub use self::audit::{ComplianceAudit, AuditType, AuditStatus, AuditFinding};
pub use self::report::{ComplianceReport, ReportType, ReportStatus, ReportSection};
pub use self::evidence::{ComplianceEvidence, EvidenceType, EvidenceStatus, EvidenceMetadata};
...
impl ComplianceManager {
    ...
    fn add_requirement(
        ...
        self.requirements.insert(requirement_id.clone(), requirement);
    ...
    pub fn get_framework(&self, framework_id: &str) -> Result<ComplianceFramework, Error> {
        self.frameworks.get(framework_id)
            .cloned()
            .ok_or_else(|| Error::NotFound(format!("Compliance framework not found: {}", framework_id)))
    }
    
    /// フレームワークリストを取得
    pub fn get_frameworks(&self, framework_type: Option<FrameworkType>) -> Vec<ComplianceFramework> {
        self.frameworks.values()
            .filter(|f| framework_type.as_ref().map_or(true, |t| f.framework_type == *t))
            .cloned()
            .collect()
    }
    ...
    pub fn get_requirement(&self, requirement_id: &str) -> Result<ComplianceRequirement, Error> {
        self.requirements.get(requirement_id)
            .cloned()
            .ok_or_else(|| Error::NotFound(format!("Compliance requirement not found: {}", requirement_id)))
    }
    
    /// フレームワークの要件リストを取得
    pub fn get_framework_requirements(&self, framework_id: &str) -> Result<Vec<ComplianceRequirement>, Error> {
        // フレームワークが存在するかチェック
        if !self.frameworks.contains_key(framework_id) {
            return Err(Error::NotFound(format!("Compliance framework not found: {}", framework_id)));
        }
        
        // フレームワークの要件IDリストを取得
        let requirement_ids = self.framework_requirements.get(framework_id)
            .cloned()
            .unwrap_or_default();
        
        // 要件リストを取得
        let requirements: Vec<ComplianceRequirement> = requirement_ids.iter()
    ...
    pub fn update_requirement_status(
        ...
        let requirement = self.requirements.get_mut(requirement_id)
            .ok_or_else(|| Error::NotFound(format!("Compliance requirement not found: {}", requirement_id)))?;
        ...
        info!("Requirement status updated: {} -> {:?}", requirement_id, status);
    ...
    pub fn add_audit_finding(
        ...
        // 監査を取得
        let audit = self.audits.get_mut(audit_id)
            .ok_or_else(|| Error::NotFound(format!("Compliance audit not found: {}", audit_id)))?;
        
        // 監査ステータスをチェック
        if audit.status != AuditStatus::InProgress {
            return Err(Error::InvalidState(format!("Audit is not in progress: {}", audit_id)));
        }
        
        // 要件が存在するかチェック
        if !self.requirements.contains_key(requirement_id) {
            return Err(Error::NotFound(format!("Compliance requirement not found: {}", requirement_id)));
        }
        ...
        info!("Audit finding added: {} for requirement {}", finding_id, requirement_id);
    ...
    /// 監査リストを取得
    pub fn get_audits(&self, framework_id: Option<&str>, status: Option<AuditStatus>) -> Vec<ComplianceAudit> {
        self.audits.values()
            .filter(|a| {
    ...
    pub fn generate_report(
        ...
        if let Some(audit_id) = audit_id {
            if !self.audits.contains_key(audit_id) {
                return Err(Error::NotFound(format!("Compliance audit not found: {}", audit_id)));
            }
        }
        ...
        // 結論セクション
        let implemented_count = requirements.iter().filter(|r| r.status == RequirementStatus::Implemented).count();
    ...
    pub fn publish_report(&mut self, report_id: &str) -> Result<(), Error> {
        ...
        if report.status != ReportStatus::Draft {
            return Err(Error::InvalidState(format!("Report is not in draft status: {}", report_id)));
        }
    ...
    /// レポートリストを取得
    pub fn get_reports(&self, framework_id: Option<&str>, status: Option<ReportStatus>) -> Vec<ComplianceReport> {
        self.reports.values()
            .filter(|r| {
    ...
    pub fn get_evidence(&self, evidence_id: &str) -> Result<ComplianceEvidence, Error> {
        self.evidence.get(evidence_id)
            .cloned()
            .ok_or_else(|| Error::NotFound(format!("Compliance evidence not found: {}", evidence_id)))
    }
    
    /// 証拠リストを取得
    pub fn get_evidence_list(&self, evidence_type: Option<EvidenceType>) -> Vec<ComplianceEvidence> {
    ...
    pub fn run_audit(&mut self, framework_id: &str) -> Result<ComplianceReport, Error> {
        ...
        let report_id = self.generate_report(
            ...
            &format!("Compliance report based on automated audit for {}", framework.name),
            framework_id,
            Some(&audit_id),
            ReportType::Audit,
            "system",
...
mod tests {
    ...
    fn test_requirement_status_update() {
        ...
        // 要件ステータスを更新
        manager.update_requirement_status(
            requirement_id,
        ...
        assert_eq!(updated_requirement.implementation_notes, Some("Implemented with data encryption".to_string()));
        assert_eq!(updated_requirement.assessed_by, Some("compliance-officer".to_string()));
    ...
    fn test_audit_and_report() {
        ...
        // 監査を作成
        let audit_id = manager.create_audit(
    ...
    fn test_evidence_management() {
        ...
        assert_eq!(evidence.content, b"{\"encryption\": \"AES-256\", \"enabled\": true}");
    }
