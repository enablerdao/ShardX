...
pub use self::alert::{AlertNotifier, AlertSeverity, AlertType, AuditAlert};
pub use self::event::{AuditEvent, AuditEventType, EventSource, EventTarget};
pub use self::log::{AuditLog, LogFormat, LogLevel, LogStorage};
pub use self::report::{AuditReport, ReportFormat, ReportGenerator, ReportPeriod};
...
impl AuditManager {
    ...
    pub fn new(config: AuditConfig) -> Self {
        ...
        manager
            .event_type_filters
            .insert(AuditEventType::Authentication, true);
        manager
            .event_type_filters
            .insert(AuditEventType::Authorization, true);
        manager
            .event_type_filters
            .insert(AuditEventType::DataAccess, true);
        manager
            .event_type_filters
            .insert(AuditEventType::SystemChange, true);
        manager
            .event_type_filters
            .insert(AuditEventType::UserActivity, true);
        manager
            .event_type_filters
            .insert(AuditEventType::SecurityEvent, true);
        manager
            .event_type_filters
            .insert(AuditEventType::ComplianceEvent, true);
        manager
            .event_type_filters
            .insert(AuditEventType::ResourceEvent, true);
        manager
            .event_type_filters
            .insert(AuditEventType::NetworkEvent, true);
        manager
            .event_type_filters
            .insert(AuditEventType::ApplicationEvent, true);
        manager
            .event_type_filters
            .insert(AuditEventType::DatabaseEvent, true);
        manager
            .event_type_filters
            .insert(AuditEventType::APIEvent, true);
        manager
            .event_type_filters
            .insert(AuditEventType::AccessControl, true);
        manager
            .event_type_filters
            .insert(AuditEventType::Other, true);
    ...
    pub fn create_audit_log(
        ...
        let log = AuditLog {
            ...
            retention_period: Duration::days(
                self.config.audit_log_retention_seconds as i64 / 86400,
            ),
            enabled: true,
    ...
    fn write_event_to_logs(&self, event: &AuditEvent) -> Result<(), Error> {
        ...
        let log_name = match event.event_type {
            ...
            | AuditEventType::UserActivity => "user",
        ...
        match event_type {
            ...
            AuditEventType::SystemChange => {
                rules.push(AlertRule {
                    ...
                    condition: "action.contains('config') || action.contains('setting')"
                        .to_string(),
    ...
    pub fn get_trail_entries(
        ...
        let mut entries: Vec<&TrailEntry> = trail
            .entries
            .iter()
            .filter(|e| {
