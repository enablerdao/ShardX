use ethers::{
    prelude::*,
    providers::{Http as EthersHttp, Provider},
...
use super::bridge::{BridgeConfig, BridgeStatus, ChainType, CrossChainBridge, FeeSetting};
...
impl EthereumBridge {
    ...
    pub fn new(
        ...
        let base_bridge = CrossChainBridge::new(config.clone(), message_sender, message_receiver);
    ...
    pub async fn initialize(&mut self, private_key: &str) -> Result<(), Error> {
        ...
        let http = Http::new(&endpoint).map_err(|e| {
            Error::ConnectionError(format!("Failed to connect to Ethereum node: {}", e))
        })?;
        ...
        // Ethersプロバイダーを初期化
        let provider = Provider::<EthersHttp>::try_from(endpoint.clone()).map_err(|e| {
            Error::ConnectionError(format!("Failed to create Ethers provider: {}", e))
        })?;
        ...
        // 接続テスト
        let block_number =
            web3.eth().block_number().await.map_err(|e| {
                Error::ConnectionError(format!("Failed to get block number: {}", e))
            })?;
        ...
        *self.latest_block.write().unwrap() = block_number_u64;
        ...
        info!(
            "Connected to Ethereum network. Latest block: {}",
            block_number_u64
        );
        ...
        *self.gas_price.write().unwrap() = gas_price;
        ...
        if let Some(addr) = self.contract_address {
            let code = web3.eth().code(addr, None).await.map_err(|e| {
                Error::ConnectionError(format!("Failed to get contract code: {}", e))
            })?;
    ...
    pub async fn send_transaction(&self, transaction: &Transaction) -> Result<H256, Error> {
        ...
        // トランザクションを送信
        let tx_hash =
            web3.eth().send_transaction(tx_request).await.map_err(|e| {
                Error::TransactionError(format!("Failed to send transaction: {}", e))
            })?;
    ...
    pub async fn check_transaction_status(
        ...
        let receipt = web3.eth().transaction_receipt(tx_hash).await.map_err(|e| {
            Error::TransactionError(format!("Failed to get transaction receipt: {}", e))
        })?;
        ...
        if let Some(receipt) = receipt {
            ...
            let block_number = receipt
                .block_number
                .ok_or_else(|| {
                    Error::TransactionError("Missing block number in receipt".to_string())
                })?
                .as_u64();
    ...
    pub async fn create_transaction_proof(
        ...
        // 証明データを作成
        let proof_data =
            serde_json::to_vec(&receipt).map_err(|e| Error::SerializationError(e.to_string()))?;
