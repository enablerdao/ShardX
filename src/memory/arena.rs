...
impl<T> ArenaBlock<T> {
    ...
    fn new(capacity: usize) -> Self {
        ...
        // メモリレイアウトを作成
        let layout = Layout::from_size_align(size * capacity, align).expect("Invalid layout");

        // メモリを割り当て
        let memory = unsafe { NonNull::new(alloc(layout)).expect("Memory allocation failed") };
    ...
    fn allocate(&mut self, value: T) -> *mut T {
        ...
        // オブジェクトのポインタを計算
        let ptr = unsafe { self.memory.as_ptr().add(offset) as *mut T };
