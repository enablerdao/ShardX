...
impl<T> MemoryPool<T> {
    ...
    pub fn new(chunk_size: Option<usize>) -> Arc<Self> {
        ...
        assert!(
            objects_per_chunk > 0,
            "Chunk size too small for object type"
        );
    ...
    fn allocate_chunk(&self) {
        ...
        // チャンクのレイアウトを作成
        let layout = Layout::from_size_align(self.chunk_size, align).expect("Invalid layout");

        // チャンクを割り当て
        let chunk = unsafe { NonNull::new(alloc(layout)).expect("Memory allocation failed") };
        ...
        for i in 0..self.objects_per_chunk {
            ...
            let node_ptr = unsafe { chunk.as_ptr().add(offset) as *mut FreeNode };
