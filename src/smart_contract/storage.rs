    ...
    fn from(error: StorageError) -> Self {
        match error {
            StorageError::KeyNotFound(key) => {
                crate::error::Error::NotFound(format!("Key not found: {}", key))
            }
            StorageError::InvalidKey(key) => {
                crate::error::Error::InvalidInput(format!("Invalid key: {}", key))
            }
            StorageError::InvalidValue(value) => {
                crate::error::Error::InvalidInput(format!("Invalid value: {}", value))
            }
            StorageError::StorageCapacityExceeded => {
                crate::error::Error::ResourceExhausted("Storage capacity exceeded".to_string())
            }
            StorageError::ContractNotFound(address) => {
                crate::error::Error::NotFound(format!("Contract not found: {}", address))
            }
            StorageError::InvalidContract(address) => {
                crate::error::Error::InvalidInput(format!("Invalid contract: {}", address))
            }
            StorageError::ContractStorageNotFound(address) => {
                crate::error::Error::NotFound(format!("Contract storage not found: {}", address))
            }
            StorageError::DatabaseError(msg) => {
                crate::error::Error::Internal(format!("Database error: {}", msg))
            }
            StorageError::SerializationError(msg) => crate::error::Error::SerializationError(msg),
            StorageError::DeserializationError(msg) => {
                crate::error::Error::DeserializationError(msg)
            }
