...
use crate::smartcontract::{Contract, ContractState, ContractFunction, ExecutionContext, ExecutionResult};
...
impl SmartContractExecutionOptimizer {
    ...
    pub fn add_execution_task(&self, task: ExecutionTask) -> Result<(), Error> {
        ...
        if let Some(depends_on) = &task.depends_on {
            ...
            let dependencies = dependency_graph.entry(task.id.clone())
        ...
        self.metrics.set_gauge("execution_queue_size", execution_queue.len() as f64);
    ...
    pub fn get_next_task(&self) -> Option<ExecutionTask> {
        ...
        while i < execution_queue.len() {
            ...
            if !executing.contains(&task.id) {
                ...
                let has_unresolved_dependencies = if let Some(dependencies) = dependency_graph.get(&task.id) {
    ...
    /// 実行モードを決定
    pub fn determine_execution_mode(&self, contract_id: &str, function_name: &str) -> ExecutionMode {
        // 実行回数を取得
    ...
    pub fn jit_compile(&self, contract: &Contract, function_name: &str) -> Result<Vec<u8>, Error> {
        ...
        let function = contract.functions.iter()
            ...
            .ok_or_else(|| Error::ContractFunctionNotFound(format!(
    ...
    /// 関数をコンパイル
    fn compile_function(&self, contract: &Contract, function: &ContractFunction) -> Result<Vec<u8>, Error> {
        ...
        // メトリクスを更新
        self.metrics.observe_histogram("jit_compilation_time", start_time.elapsed().as_secs_f64());
    ...
    where
        F: Fn(ExecutionTask, ExecutionMode) -> Result<ExecutionResult, Error> + Send + Sync + 'static,
    {
        // 既に実行中かチェック
        let mut running = self.running.lock().unwrap();
        if *running {
            return Err(Error::InvalidState("Execution optimizer is already running".to_string()));
        }
                    ...
                    tokio::spawn(async move {
                        ...
                        // 実行回数をインクリメント
                        {
                            ...
                            *count += 1;
                            ...
                            if *count >= hot_path_threshold {
                                ...
                                let paths = hot_paths.entry(task.contract_id.clone()).or_insert_with(Vec::new);
                        ...
                        match result {
                            Ok(Ok(execution_result)) => {
                                ...
                                // メトリクスを更新
                                metrics.observe_histogram("execution_time", execution_time.as_secs_f64());
