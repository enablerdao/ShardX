...
impl<S: ContractStorage> WasmVM<S> {
    ...
    fn load_module(&mut self, address: &str) -> Result<WasmModule, VMError> {
        ...
        // キャッシュに保存
        self.module_cache
            .insert(address.to_string(), module.clone());
...
impl<S: ContractStorage> VirtualMachine for WasmVM<S> {
    ...
    fn delete(
        ...
        // コントラクトをストレージから削除
        self.storage
            .delete_contract(&address)
            .map_err(|e| VMError::InternalError(format!("Failed to delete contract: {}", e)))?;

        // コントラクトストレージをクリア
        self.storage.clear_contract_storage(&address).map_err(|e| {
            VMError::InternalError(format!("Failed to clear contract storage: {}", e))
        })?;
