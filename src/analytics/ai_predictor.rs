...
impl TransactionPredictor {
    ...
    pub fn add_transaction_data(&mut self, transactions: &[Transaction], timestamp: DateTime<Utc>) {
        ...
        // トランザクション量
        let transaction_volume: f64 = transactions.iter().map(|tx| tx.amount as f64).sum();
        self.add_feature_data("transaction_volume", timestamp, transaction_volume);

        // 平均手数料
        let total_fees: f64 = transactions.iter().map(|tx| tx.fee as f64).sum();
        ...
        // アクティブユーザー
        let unique_users: std::collections::HashSet<_> = transactions
            .iter()
    ...
    pub fn train(&mut self) -> Result<(), Error> {
        ...
        // 学習期間のデータを取得
        let training_start =
            now - chrono::Duration::seconds(self.config.training_period_seconds as i64);
        ...
        // ターゲットデータを準備
        let filtered_target: Vec<_> = self
            .target_data
            .iter()
            .filter(|(ts, _)| *ts >= training_start)
            .collect();
    ...
    pub fn predict(&mut self) -> Result<Vec<PredictionResult>, Error> {
        ...
        let model = self
            .model
            .as_ref()
            .ok_or_else(|| Error::InvalidState("Model not trained".to_string()))?;

        // 予測期間を計算
        let prediction_end =
            now + chrono::Duration::seconds(self.config.prediction_period_seconds as i64);
    ...
    pub fn evaluate_accuracy(&self) -> Result<f64, Error> {
        ...
        for prediction in &self.prediction_history {
            // 予測対象時刻に最も近い実際の値を検索
            let actual = self.target_data.iter().min_by_key(|(ts, _)| {
