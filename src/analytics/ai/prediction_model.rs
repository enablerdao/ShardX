...
use crate::analytics::ai_predictor::{PredictionConfig, PredictionModel, PredictionResult};
...
impl AdvancedPredictionEngine {
    ...
    pub fn train(&mut self, target: Option<PredictionTarget>) -> Result<(), Error> {
        ...
        for target in targets {
            ...
            // 学習期間のデータを取得
            let training_start =
                now - Duration::seconds(self.config.training_period_seconds as i64);
            ...
            for feature_name in &self.config.features {
                if let Some(feature_data) = self.feature_data.get(feature_name) {
                    let filtered_data: Vec<_> = feature_data
                        .iter()
                    ...
                    features.insert(
                        feature_name.clone(),
                        filtered_data.iter().map(|(_, v)| *v).collect(),
                    );
                } else {
            ...
            if features.is_empty() {
                return Err(Error::InvalidState(
                    "No feature data available for training".to_string(),
                ));
            }
            ...
            let target_data = self
                ...
                .entry(target.clone())
                .or_insert_with(Vec::new);
            let filtered_target: Vec<_> = target_data
                .iter()
                .filter(|(ts, _)| *ts >= training_start)
                .collect();
            ...
            model.train(
                &features,
                &target_values,
                self.config.hyperparameters.as_ref(),
            )?;
    ...
    fn get_or_create_model(
        &mut self,
        target: &PredictionTarget,
    ) -> &mut Box<dyn PredictionModelTrait> {
    ...
    pub fn predict(
    ...
    ) -> Result<PredictionResultSet, Error> {
        let now = Utc::now();
        ...
        if intervals == 0 {
            return Err(Error::InvalidInput(
                "Prediction horizon must be greater than interval".to_string(),
            ));
        }
        ...
        if latest_features.is_empty() {
            return Err(Error::InvalidState(
                "No feature data available for prediction".to_string(),
            ));
        }
        ...
        for i in 1..=intervals {
            ...
            let (lower_bound, upper_bound) =
                if let Some(confidence_interval) = self.config.confidence_interval {
        ...
        let prediction_id = format!(
            "prediction_{}_{}",
            target_timestamp_to_string(&now),
    ...
    pub fn evaluate(
        &mut self,
        target: Option<PredictionTarget>,
    ) -> Result<HashMap<EvaluationMetric, f64>, Error> {
        let target = target.unwrap_or_else(|| self.config.target.clone());
        ...
        let target_data = self
            .target_data
            .get(&target)
            .ok_or_else(|| Error::NotFound(format!("No target data for: {:?}", target)))?;

        if target_data.is_empty() {
            return Err(Error::InvalidState(
                "No target data to evaluate against".to_string(),
            ));
        }
        ...
        for point in &latest_prediction.results {
            // 予測対象時刻に最も近い実際の値を検索
            let actual = target_data.iter().min_by_key(|(ts, _)| {
        ...
        if count == 0 {
            return Err(Error::InvalidState(
                "No matching data points for evaluation".to_string(),
            ));
        }
    ...
    pub fn get_prediction_history(
        &self,
        target: Option<PredictionTarget>,
    ) -> Option<&Vec<PredictionResultSet>> {
        let target = target.unwrap_or_else(|| self.config.target.clone());
    ...
    pub fn get_evaluation_results(
        &self,
        target: &PredictionTarget,
    ) -> Option<&HashMap<EvaluationMetric, Vec<f64>>> {
        self.evaluation_results.get(target)
    }

    /// 最新の評価結果を取得
    pub fn get_latest_evaluation(
        &self,
        target: &PredictionTarget,
    ) -> Option<HashMap<EvaluationMetric, f64>> {
    ...
    pub fn detect_anomalies(
        ...
        if target_data.is_empty() {
            return Err(Error::InvalidState(
                "No target data for anomaly detection".to_string(),
            ));
        }
        ...
        // 期間内のデータをフィルタリング
        let filtered_data: Vec<_> = target_data
            .iter()
        ...
        if filtered_data.is_empty() {
            return Err(Error::InvalidState(
                "No data in lookback period for anomaly detection".to_string(),
            ));
        }
    ...
    pub fn detect_changepoints(
        ...
        if target_data.is_empty() {
            return Err(Error::InvalidState(
                "No target data for changepoint detection".to_string(),
            ));
        }
        ...
        if filtered_data.len() < 10 {
            return Err(Error::InvalidState(
                "Insufficient data for changepoint detection".to_string(),
            ));
        }
        ...
        for i in window_size..(filtered_data.len() - window_size) {
            let before_avg = filtered_data[i - window_size..i]
                .iter()
                .map(|(_, v)| *v)
                .sum::<f64>()
                / window_size as f64;

            let after_avg = filtered_data[i..i + window_size]
                .iter()
                .map(|(_, v)| *v)
                .sum::<f64>()
                / window_size as f64;
...
impl PredictionModelTrait for LinearRegressionModel {
    fn train(
        ...
        features: &HashMap<String, Vec<f64>>,
        target: &[f64],
        _hyperparameters: Option<&HashMap<String, serde_json::Value>>,
    ) -> Result<(), Error> {
...
impl PredictionModelTrait for ARIMAModel {
    fn train(
        ...
        let d = hyperparameters
            .and_then(|h| h.get("d"))
            .and_then(|v| v.as_u64())
            .unwrap_or(0) as usize;
        let q = hyperparameters
            .and_then(|h| h.get("q"))
            .and_then(|v| v.as_u64())
            .unwrap_or(1) as usize;
