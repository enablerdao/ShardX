...
impl TransactionAnalyzer {
    ...
    pub fn new(
        ...
        let optimization_engine = if config.optimization_suggestions {
            ...
            engine.optimization_rules.push(OptimizationRule {
                ...
                suggestion: "シャード間の通信を最適化することで、レイテンシを削減できます。".to_string(),
            });
    ...
    /// トランザクションを分析
    pub fn analyze_transaction(&self, transaction: &Transaction) -> Result<TransactionAnalysis, Error> {
        ...
        // 処理時間を計算
        let processing_time_ms = if let (Some(first), Some(last)) = (processing_steps.first(), processing_steps.last()) {
        ...
        // 異常を検出
        let (anomaly_detected, anomaly_details) = self.detect_anomalies(transaction, &processing_steps)?;
        
        // パフォーマンススコアを計算
        let performance_score = self.calculate_performance_score(transaction, processing_time_ms, &shards_traversed)?;
        ...
        if self.config.detailed_analysis {
            ...
            // 古いエントリを削除
            let retention_time = chrono::Duration::seconds(self.config.history_retention_seconds as i64);
    ...
    /// 処理ステップを収集
    fn collect_processing_steps(&self, transaction: &Transaction) -> Result<Vec<ProcessingStep>, Error> {
    ...
    /// コストを計算
    fn calculate_cost(&self, transaction: &Transaction, steps: &[ProcessingStep]) -> Result<f64, Error> {
        // 実際の実装では、トランザクションの複雑さや処理時間からコストを計算
        // ここでは簡易的な実装として、ダミーデータを返す
        
        let base_cost = 0.001; // 基本コスト
        let time_cost = steps.iter().map(|s| s.duration_ms as f64 * 0.0001).sum::<f64>(); // 時間コスト
    ...
    /// 異常を検出
    fn detect_anomalies(&self, transaction: &Transaction, steps: &[ProcessingStep]) -> Result<(bool, Option<String>), Error> {
        // 異常検出が無効の場合は何もしない
        ...
        if total_time > detector.processing_time_threshold_ms {
            return Ok((true, Some(format!("処理時間が閾値を超えています: {}ms", total_time))));
        }
        
        // エラーステップを検出
        let error_steps = steps.iter().filter(|s| s.status == StepStatus::Error).count();
        if error_steps > 0 {
            return Ok((true, Some(format!("{}個のエラーステップがあります", error_steps))));
        }
    ...
    /// パフォーマンススコアを計算
    fn calculate_performance_score(&self, transaction: &Transaction, processing_time_ms: u64, shards_traversed: &[ShardId]) -> Result<u8, Error> {
    ...
    /// メタデータを収集
    fn collect_metadata(&self, transaction: &Transaction) -> Result<HashMap<String, String>, Error> {
        ...
        metadata.insert("transaction_type".to_string(), format!("{:?}", transaction.transaction_type));
    ...
    fn update_statistics(&self, analysis: &TransactionAnalysis) -> Result<(), Error> {
        ...
        stats.avg_processing_time_ms = (old_avg * old_count as f64 + analysis.processing_time_ms as f64) / stats.transactions_analyzed as f64;
        stats.max_processing_time_ms = stats.max_processing_time_ms.max(analysis.processing_time_ms);
        if analysis.processing_time_ms > 0 {
            stats.min_processing_time_ms = stats.min_processing_time_ms.min(analysis.processing_time_ms);
        }
        
        // 成功率を更新
        let success_count = if analysis.status == TransactionStatus::Confirmed { 1 } else { 0 };
        let old_success_rate = stats.success_rate;
        stats.success_rate = (old_success_rate * old_count as f64 + success_count as f64) / stats.transactions_analyzed as f64 * 100.0;
        ...
        stats.avg_performance_score = (old_avg_score * old_count as f64 + analysis.performance_score as f64) / stats.transactions_analyzed as f64;
    ...
    /// トランザクションフローを取得
    pub fn get_transaction_flow(&self, transaction_id: &str) -> Result<Option<TransactionFlow>, Error> {
        // 履歴から分析結果を取得
        ...
        let flow = TransactionFlow {
            ...
            amount: 1.0,
            from_shard: analysis.shards_traversed.first().cloned().unwrap_or_default(),
            to_shard: analysis.shards_traversed.last().cloned().unwrap_or_default(),
            via_shards: analysis.shards_traversed.clone(),
            start_time: analysis.processing_steps.first().map(|s| s.start_time).unwrap_or_else(Utc::now),
            end_time: analysis.processing_steps.last().map(|s| s.end_time).unwrap_or_else(Utc::now),
