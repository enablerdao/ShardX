...
impl CrossShardAnalyzer {
    ...
    pub fn create_cross_shard_transaction(&self, transaction: &Transaction, source_shard_id: ShardId, target_shard_id: ShardId) -> CrossShardTransaction {
    ...
    /// クロスシャードトランザクションを更新
    pub fn update_cross_shard_transaction(&mut self, transaction_id: &str, status: TransactionStatus, completion_time: Option<DateTime<Utc>>, latency_ms: Option<u64>) -> Result<(), Error> {
        let transaction = self.cross_shard_transactions.iter_mut()
            .find(|tx| tx.transaction_id == transaction_id)
            .ok_or_else(|| Error::NotFound(format!("Cross-shard transaction not found: {}", transaction_id)))?;
    ...
    pub fn add_shard_latency(&mut self, latency: ShardLatency) {
        let key = (latency.source_shard_id.clone(), latency.target_shard_id.clone());
    ...
    pub fn add_shard_throughput(&mut self, throughput: ShardThroughput) {
        let key = (throughput.source_shard_id.clone(), throughput.target_shard_id.clone());
    ...
    /// クロスシャードトランザクションを分析
    pub fn analyze_transactions(&self, start_time: DateTime<Utc>, end_time: DateTime<Utc>) -> CrossShardAnalysisResult {
        // 期間内のトランザクションをフィルタリング
        let transactions: Vec<_> = self.cross_shard_transactions.iter()
        ...
        // 成功したトランザクション数
        let successful_transactions = transactions.iter()
        ...
        // 平均レイテンシ
        let latencies: Vec<_> = transactions.iter()
            .filter_map(|tx| tx.latency_ms)
            .collect();
    ...
    /// シャード間レイテンシを分析
    pub fn analyze_latencies(&self, start_time: DateTime<Utc>, end_time: DateTime<Utc>) -> HashMap<(ShardId, ShardId), ShardLatencyAnalysis> {
        let mut result = HashMap::new();
        
        for ((source, target), latencies) in &self.shard_latencies {
            // 期間内のレイテンシをフィルタリング
            let filtered_latencies: Vec<_> = latencies.iter()
            ...
            // 平均レイテンシ
            let average_latency = filtered_latencies.iter()
                .map(|l| l.latency_ms)
                .sum::<u64>() as f64 / filtered_latencies.len() as f64;
            
            // 最小レイテンシ
            let min_latency = filtered_latencies.iter()
                .map(|l| l.min_latency_ms)
                .min()
                .unwrap_or(0);
            
            // 最大レイテンシ
            let max_latency = filtered_latencies.iter()
                .map(|l| l.max_latency_ms)
                .max()
                .unwrap_or(0);
            
            // 標準偏差
            let std_dev = filtered_latencies.iter()
                .map(|l| l.std_dev_ms)
                .sum::<f64>() / filtered_latencies.len() as f64;
    ...
    /// シャード間スループットを分析
    pub fn analyze_throughputs(&self, start_time: DateTime<Utc>, end_time: DateTime<Utc>) -> HashMap<(ShardId, ShardId), ShardThroughputAnalysis> {
        let mut result = HashMap::new();
        
        for ((source, target), throughputs) in &self.shard_throughputs {
            // 期間内のスループットをフィルタリング
            let filtered_throughputs: Vec<_> = throughputs.iter()
            ...
            // 平均TPS
            let average_tps = filtered_throughputs.iter()
                .map(|t| t.tps)
                .sum::<f64>() / filtered_throughputs.len() as f64;
            
            // 最大TPS
            let max_tps = filtered_throughputs.iter()
                .map(|t| t.tps)
                .fold(0.0, f64::max);
            
            // 平均バイト/秒
            let average_bytes_per_second = filtered_throughputs.iter()
                .map(|t| t.bytes_per_second)
                .sum::<f64>() / filtered_throughputs.len() as f64;
            
            // 平均成功率
            let average_success_rate = filtered_throughputs.iter()
                .map(|t| t.success_rate)
                .sum::<f64>() / filtered_throughputs.len() as f64;
            
            // 総トランザクション数
            let total_transactions = filtered_throughputs.iter()
                .map(|t| t.transaction_count)
                .sum::<u64>();
            
            // 総トランザクション量
            let total_volume = filtered_throughputs.iter()
    ...
    /// シャード間フローを分析
    pub fn analyze_flows(&self, start_time: DateTime<Utc>, end_time: DateTime<Utc>) -> HashMap<(ShardId, ShardId), ShardFlowAnalysis> {
        let mut result = HashMap::new();
        
        for ((source, target), flows) in &self.shard_flows {
            // 期間内のフローをフィルタリング
            let filtered_flows: Vec<_> = flows.iter()
            ...
            // 総送信トランザクション数
            let total_outgoing_transactions = filtered_flows.iter()
                .map(|f| f.outgoing_transactions)
                .sum::<u64>();
            
            // 総受信トランザクション数
            let total_incoming_transactions = filtered_flows.iter()
                .map(|f| f.incoming_transactions)
                .sum::<u64>();
            
            // 総送信トランザクション量
            let total_outgoing_volume = filtered_flows.iter()
                .map(|f| f.outgoing_volume)
                .sum::<u64>();
            
            // 総受信トランザクション量
            let total_incoming_volume = filtered_flows.iter()
                .map(|f| f.incoming_volume)
                .sum::<u64>();
            
            // 純フロー（トランザクション数）
            let net_transaction_flow = total_outgoing_transactions as i64 - total_incoming_transactions as i64;
    ...
    /// クロスシャードネットワークを分析
    pub fn analyze_network(&self, start_time: DateTime<Utc>, end_time: DateTime<Utc>) -> CrossShardNetworkAnalysis {
        // トランザクション分析
        ...
        for (shard_id, info) in &self.shard_info {
            nodes.insert(shard_id.clone(), ShardNode {
        ...
        for ((source, target), count) in &transaction_analysis.transactions_by_shard_pair {
            let latency = latency_analysis.get(&(source.clone(), target.clone()))
                .map(|l| l.average_latency)
                .unwrap_or(0.0);
            
            let throughput = throughput_analysis.get(&(source.clone(), target.clone()))
                .map(|t| t.average_tps)
                .unwrap_or(0.0);
            
            let success_rate = throughput_analysis.get(&(source.clone(), target.clone()))
            ...
            if let Some(node) = nodes.get_mut(source) {
                ...
                node.outgoing_transactions += flow.map(|f| f.total_outgoing_transactions).unwrap_or(0);
                node.outgoing_volume += flow.map(|f| f.total_outgoing_volume).unwrap_or(0);
            }
            
            if let Some(node) = nodes.get_mut(target) {
                node.transaction_count += *count;
                node.incoming_transactions += flow.map(|f| f.total_incoming_transactions).unwrap_or(0);
    ...
    /// シャードレイテンシを取得
    pub fn get_shard_latencies(&self, source_shard_id: &ShardId, target_shard_id: &ShardId) -> Option<&[ShardLatency]> {
        self.shard_latencies.get(&(source_shard_id.clone(), target_shard_id.clone()))
            .map(|latencies| latencies.as_slice())
    }
    
    /// シャードスループットを取得
    pub fn get_shard_throughputs(&self, source_shard_id: &ShardId, target_shard_id: &ShardId) -> Option<&[ShardThroughput]> {
        self.shard_throughputs.get(&(source_shard_id.clone(), target_shard_id.clone()))
            .map(|throughputs| throughputs.as_slice())
    }
    
    /// シャードフローを取得
    pub fn get_shard_flows(&self, source_shard_id: &ShardId, target_shard_id: &ShardId) -> Option<&[ShardFlow]> {
        self.shard_flows.get(&(source_shard_id.clone(), target_shard_id.clone()))
