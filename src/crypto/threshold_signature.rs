...
impl ThresholdSignature {
    ...
    pub fn generate_shares(&self, secret: &BigUint) -> Result<Vec<SignatureShare>, Error> {
        if secret >= &self.prime {
            return Err(Error::CryptoError(
                "Secret must be less than prime".to_string(),
            ));
        }
        ...
        for i in 1..=self.total_shares {
            ...
            shares.push(SignatureShare { id: i, value: y });
        }
        ...
        for i in 0..self.threshold {
            ...
            for j in 0..self.threshold {
                ...
                if !diff.is_one() && diff.gcd(&self.prime).is_one() {
                    diff = mod_inverse(&diff, &self.prime).ok_or_else(|| {
                        Error::CryptoError("Failed to compute modular inverse".to_string())
                    })?;
                }
...
impl ThresholdVerifier {
    ...
    pub fn combine_signatures(
        ...
        // 署名シェアを抽出
        let shares: Vec<SignatureShare> = partial_signatures
            .iter()
...
mod tests {
    ...
    fn test_threshold_signature_different_subsets() {
        ...
        let reconstructed1 = scheme
            .reconstruct_secret(&[shares[0].clone(), shares[1].clone()])
            .unwrap();
        let reconstructed2 = scheme
            .reconstruct_secret(&[shares[0].clone(), shares[2].clone()])
            .unwrap();
        let reconstructed3 = scheme
            .reconstruct_secret(&[shares[1].clone(), shares[3].clone()])
            .unwrap();
    ...
    fn test_threshold_signer_and_verifier() {
        ...
        let signature = verifier
            .combine_signatures(&[partial_sig1, partial_sig2])
            .unwrap();
