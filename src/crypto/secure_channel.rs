...
impl SecureChannel {
    /// 新しいSecureChannelを作成
    pub fn new(role: Role, local_private_key: &[u8], remote_public_key: Option<&[u8]>) -> Result<Self, Error> {
        // Noiseプロトコルのパターンを設定
        // XX: 相互認証と前方秘匿性を提供
        ...
        let handshake = match role {
            ...
            Role::Responder => {
                builder.local_private_key(&local_keypair.private)
                    .build_responder()
        ...
        T: AsyncRead + AsyncWrite + Unpin,
    {
        ...
        let _payload_len = handshake.read_message(&buffer[..len], &mut buffer)
        ...
        let transport = handshake.into_transport_mode()
            .map_err(|e| Error::CryptoError(format!("Failed to transition to transport mode: {}", e)))?;
        ...
        T: AsyncWrite + Unpin,
    {
        ...
        // メッセージを暗号化
        let len = transport.write_message(message, &mut buffer)
        ...
        T: AsyncRead + Unpin,
    {
        ...
        let payload_len = transport.read_message(&buffer, &mut payload)
...
impl SecureChannelFactory {
    ...
    pub fn create_initiator(&self, remote_public_key: Option<&[u8]>) -> Result<SecureChannel, Error> {
        SecureChannel::new(Role::Initiator, &self.local_keypair.private, remote_public_key)
    }
...
mod tests {
    ...
    async fn test_secure_channel_handshake() {
        ...
        let initiator_task = tokio::spawn(async move {
            initiator.perform_handshake(&mut client.clone()).await.unwrap();
            initiator
        });
        
        let responder_task = tokio::spawn(async move {
            responder.perform_handshake(&mut server.clone()).await.unwrap();
            responder
        });
