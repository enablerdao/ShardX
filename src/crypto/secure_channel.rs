...
mod tests {
    ...
    async fn test_secure_channel_with_predefined_keys() {
        ...
        let initiator_factory =
            SecureChannelFactory::new(Some(&initiator_keypair.private)).unwrap();
        let responder_factory =
            SecureChannelFactory::new(Some(&responder_keypair.private)).unwrap();

        // 公開鍵を確認
        assert_eq!(
            *initiator_factory.local_public_key(),
            initiator_keypair.public
        );
        assert_eq!(
            *responder_factory.local_public_key(),
        ...
        let mut responder = responder_factory.create_responder().unwrap();
    ...
    async fn test_error_handling() {
        ...
        let mut channel =
            SecureChannel::new(Role::Initiator, &initiator_keypair.private, None).unwrap();
