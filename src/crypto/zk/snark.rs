...
impl ZeroKnowledgeProofSystem for Snark {
    ...
    fn verify(
        ...
        // 証明を検証
        let result =
            ark_groth16::verify_proof(&verification_key.inner, &proof.inner, &public_fr)
    ...
    fn generate_keys(
        ...
        Ok((
            SnarkProvingKey { inner: pk },
            SnarkVerificationKey { inner: vk },
        ))
    }
    ...
    fn serialize_proof(proof: &Self::Proof) -> Result<Vec<u8>, Error> {
        ...
        proof.inner.serialize(&mut bytes).map_err(|e| {
            Error::SerializationError(format!("Failed to serialize SNARK proof: {}", e))
        })?;
        Ok(bytes)
    }
    ...
    fn deserialize_proof(data: &[u8]) -> Result<Self::Proof, Error> {
        let inner = Proof::deserialize(data).map_err(|e| {
            Error::DeserializationError(format!("Failed to deserialize SNARK proof: {}", e))
        })?;
        Ok(SnarkProof { inner })
    }
    ...
    fn serialize_verification_key(key: &Self::VerificationKey) -> Result<Vec<u8>, Error> {
        ...
        key.inner.serialize(&mut bytes).map_err(|e| {
            Error::SerializationError(format!("Failed to serialize SNARK verification key: {}", e))
        })?;
        Ok(bytes)
    }
    ...
    fn deserialize_verification_key(data: &[u8]) -> Result<Self::VerificationKey, Error> {
        let inner = VerifyingKey::deserialize(data).map_err(|e| {
            Error::DeserializationError(format!(
                "Failed to deserialize SNARK verification key: {}",
                e
            ))
        })?;
