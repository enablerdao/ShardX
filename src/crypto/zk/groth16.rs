...
impl ZeroKnowledgeProofSystem for Groth16 {
    ...
    fn prove(
        ...
        // 証明を生成
        let proof =
            ark_groth16::create_random_proof(circuit, &proving_key.inner, &mut rand::thread_rng())
                .map_err(|e| {
                    Error::CryptoError(format!("Failed to create Groth16 proof: {}", e))
                })?;
    ...
    fn verify(
        ...
        let result = ark_groth16::verify_proof(&verification_key.inner, &proof.inner, &public_fr)
            .map_err(|e| {
            Error::CryptoError(format!("Failed to verify Groth16 proof: {}", e))
        })?;
    ...
    fn generate_keys(
        ...
        Ok((
            Groth16ProvingKey { inner: pk },
            Groth16VerificationKey { inner: vk },
        ))
    }

    fn serialize_proof(proof: &Self::Proof) -> Result<Vec<u8>, Error> {
        let mut bytes = Vec::new();
        proof.inner.serialize(&mut bytes).map_err(|e| {
            Error::SerializationError(format!("Failed to serialize Groth16 proof: {}", e))
        })?;
        Ok(bytes)
    }

    fn deserialize_proof(data: &[u8]) -> Result<Self::Proof, Error> {
        let inner = Proof::deserialize(data).map_err(|e| {
            Error::DeserializationError(format!("Failed to deserialize Groth16 proof: {}", e))
        })?;
    ...
    fn serialize_verification_key(key: &Self::VerificationKey) -> Result<Vec<u8>, Error> {
        let mut bytes = Vec::new();
        key.inner.serialize(&mut bytes).map_err(|e| {
            Error::SerializationError(format!(
                "Failed to serialize Groth16 verification key: {}",
                e
            ))
        })?;
        Ok(bytes)
    ...
    fn deserialize_verification_key(data: &[u8]) -> Result<Self::VerificationKey, Error> {
        let inner = VerifyingKey::deserialize(data).map_err(|e| {
            Error::DeserializationError(format!(
                "Failed to deserialize Groth16 verification key: {}",
                e
            ))
        })?;
...
impl Groth16 {
    ...
    fn bytes_to_fr(bytes: &[u8]) -> Result<Fr, Error> {
        if bytes.len() > 32 {
            return Err(Error::InvalidArgument(
                "Input too large for field element".to_string(),
            ));
        }
    ...
    fn deserialize_circuit_params(data: &[u8]) -> Result<CircuitParameters, Error> {
        ...
        let params: CircuitParameters = bincode::deserialize(data).map_err(|e| {
            Error::DeserializationError(format!("Failed to deserialize circuit parameters: {}", e))
        })?;
