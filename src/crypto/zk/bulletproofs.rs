...
use bulletproofs::{
    BulletproofGens, PedersenGens, RangeProof as BPRangeProof,
};
...
impl Bulletproof {
    ...
    pub fn verify_range(proof: &BulletproofProof, commitment_bytes: &[u8]) -> Result<bool, Error> {
        ...
        // 範囲証明を検証
        let result = proof.inner.verify_single(
            &bp_gens,
            &pc_gens,
            &mut transcript,
            &commitment,
    ...
    pub fn verify_complete_range_proof(range_proof: &RangeProof) -> Result<bool, Error> {
        ...
        // 範囲証明を検証
        let result = range_proof.proof.inner.verify_single(
            &bp_gens,
            &pc_gens,
            &mut transcript,
    ...
    fn bytes_to_scalar(bytes: &[u8]) -> Result<Scalar, Error> {
        if bytes.len() != 32 {
            return Err(Error::InvalidArgument("Blinding must be 32 bytes".to_string()));
        }
    ...
    pub fn deserialize_proof(data: &[u8]) -> Result<BulletproofProof, Error> {
        let inner = BPRangeProof::from_bytes(data)
            .map_err(|_| Error::DeserializationError("Failed to deserialize Bulletproof".to_string()))?;
    ...
    pub fn batch_prove_range(
        ...
        if values.len() != blindings.len() {
            return Err(Error::InvalidArgument("Number of values and blindings must match".to_string()));
        }
    ...
    pub fn batch_verify_range(
        ...
        if proofs.len() != commitments.len() {
            return Err(Error::InvalidArgument("Number of proofs and commitments must match".to_string()));
        }
