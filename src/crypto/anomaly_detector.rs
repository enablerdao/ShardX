...
pub trait AnomalyModel {
    /// 異常を検出
    fn detect(&self, metrics: &TransactionMetrics, history: &[TransactionMetrics]) -> HashMap<AnomalyType, f32>;
}
...
impl AnomalyModel for StatisticalModel {
    fn detect(&self, metrics: &TransactionMetrics, history: &[TransactionMetrics]) -> HashMap<AnomalyType, f32> {
        ...
        // トランザクション数の統計
        let tx_counts: Vec<usize> = recent_history.iter()
        ...
        // 異常なトランザクション量を検出
        let tx_count_z_score = z_score(metrics.transaction_count as f32, tx_count_mean, tx_count_std_dev);
        ...
        // 拒否されたトランザクション数の統計
        let rejected_counts: Vec<usize> = recent_history.iter()
        ...
        if rejected_count_z_score > self.std_dev_multiplier * 1.5 {
            let confidence = confidence_from_z_score(rejected_count_z_score, self.std_dev_multiplier * 1.5);
            anomalies.insert(AnomalyType::MaliciousActivity, confidence);
        }
        
        // 処理時間の統計
        let processing_times: Vec<f32> = recent_history.iter()
        ...
        if processing_time_z_score > self.std_dev_multiplier * 2.0 {
            let confidence = confidence_from_z_score(processing_time_z_score, self.std_dev_multiplier * 2.0);
            anomalies.insert(AnomalyType::UnusualLatency, confidence);
        }
        
        // シャード間トランザクション数の統計
        let cross_shard_counts: Vec<usize> = recent_history.iter()
        ...
        if cross_shard_z_score < -self.std_dev_multiplier * 3.0 {
            let confidence = confidence_from_z_score(-cross_shard_z_score, self.std_dev_multiplier * 3.0);
...
impl MachineLearningModel {
    ...
    fn extract_features(&self, metrics: &TransactionMetrics) -> HashMap<String, f32> {
        ...
        features.insert("transaction_count".to_string(), metrics.transaction_count as f32);
        features.insert("avg_transaction_size".to_string(), metrics.avg_transaction_size as f32);
        features.insert("avg_processing_time".to_string(), metrics.avg_processing_time.as_secs_f32());
        features.insert("rejected_transaction_count".to_string(), metrics.rejected_transaction_count as f32);
        features.insert("unique_address_count".to_string(), metrics.unique_address_count as f32);
        features.insert("cross_shard_transaction_count".to_string(), metrics.cross_shard_transaction_count as f32);
...
impl AnomalyModel for MachineLearningModel {
    fn detect(&self, metrics: &TransactionMetrics, _history: &[TransactionMetrics]) -> HashMap<AnomalyType, f32> {
        let mut anomalies = HashMap::new();
        ...
        if malicious_activity_confidence > 0.65 {
            anomalies.insert(AnomalyType::MaliciousActivity, malicious_activity_confidence);
        }
        ...
        if resource_exhaustion_confidence > 0.7 {
            anomalies.insert(AnomalyType::ResourceExhaustion, resource_exhaustion_confidence);
        }
...
impl AnomalyDetector {
    ...
    /// 緩和ルールを追加
    pub fn add_mitigation_rule(&mut self, anomaly_type: AnomalyType, min_confidence: f32, action: MitigationAction) {
    ...
    /// 監視ループを開始
    pub async fn start_monitoring_loop(detector: Arc<Self>, metrics_provider: Arc<dyn MetricsProvider + Send + Sync>) -> Result<(), Error> {
...
mod tests {
    ...
    fn test_machine_learning_model() {
        ...
        assert!(malicious_anomalies.get(&AnomalyType::MaliciousActivity).unwrap_or(&0.0) >
               normal_anomalies.get(&AnomalyType::MaliciousActivity).unwrap_or(&0.0));
    }
