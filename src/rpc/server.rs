        ...
        io.add_method("getBlock", move |params: Params| {
            ...
            async move {
                let hash_or_height = match params.parse::<Value>() {
                    ...
                    _ => return Err(JsonRpcError::invalid_params("Expected block hash or height")),
                };
        ...
        io.add_method("sendTransaction", move |params: Params| {
            ...
            async move {
                let tx: Transaction = match params.parse() {
                    Ok(tx) => tx,
                    Err(_) => return Err(JsonRpcError::invalid_params("Invalid transaction format")),
                };
        ...
        io.add_method("createMultisigWallet", move |params: Params| {
            ...
            async move {
                ...
                let wallet_params: CreateWalletParams = match params.parse() {
                    Ok(p) => p,
                    Err(_) => return Err(JsonRpcError::invalid_params("Invalid wallet parameters")),
                };
        ...
        io.add_method("createMultisigTransaction", move |params: Params| {
            ...
            async move {
                ...
                let tx_params: CreateTxParams = match params.parse() {
                    Ok(p) => p,
                    Err(_) => return Err(JsonRpcError::invalid_params("Invalid transaction parameters")),
                };
        ...
        io.add_method("signMultisigTransaction", move |params: Params| {
            ...
            async move {
                ...
                let sign_params: SignTxParams = match params.parse() {
                    Ok(p) => p,
                    Err(_) => return Err(JsonRpcError::invalid_params("Invalid signature parameters")),
                };
        ...
        io.add_method("getPricePrediction", move |params: Params| {
            ...
            async move {
                ...
                let pred_params: PredictionParams = match params.parse() {
                    Ok(p) => p,
                    Err(_) => return Err(JsonRpcError::invalid_params("Invalid prediction parameters")),
                };
                
                match get_price_prediction(
                    node,
                    &pred_params.pair,
                    &pred_params.period,
                ).await {
        ...
        io.add_method("getTradingRecommendation", move |params: Params| {
            ...
            async move {
                ...
                let rec_params: RecommendationParams = match params.parse() {
                    Ok(p) => p,
                    Err(_) => return Err(JsonRpcError::invalid_params("Invalid recommendation parameters")),
                };
                
                match get_trading_recommendation(
                    node,
                    &rec_params.pair,
                ).await {
...
async fn get_block(node: Arc<Node>, hash_or_height: &str) -> Result<BlockInfo, Error> {
    ...
    Ok(BlockInfo {
        ...
        previous_hash: "0xabcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890".to_string(),
        height: 12345,
...
async fn get_transactions(node: Arc<Node>, address: &str) -> Result<Vec<Transaction>, Error> {
    ...
    for i in 0..10 {
        let tx = Transaction {
            id: format!("tx{}", i),
            from: if i % 2 == 0 { address.to_string() } else { "0xabcdef1234567890abcdef1234567890abcdef12".to_string() },
            to: if i % 2 == 0 { "0xabcdef1234567890abcdef1234567890abcdef12".to_string() } else { address.to_string() },
...
async fn sign_multisig_transaction(
    ...
    let mut signatures = std::collections::HashMap::new();
    signatures.insert("0x1234567890abcdef1234567890abcdef12345678".to_string(), "0xsig1".to_string());
...
async fn get_chart_data(
    ...
    let chart_metric = match metric {
        ...
        _ => return Err(Error::ValidationError(format!("Invalid chart metric: {}", metric))),
    };
    ...
    let chart_period = match period {
        ...
        "all" => ChartPeriod::All,
        _ => return Err(Error::ValidationError(format!("Invalid chart period: {}", period))),
    };
    
    // 開始時刻と終了時刻を設定
    let end = end_time.map(|t| chrono::DateTime::<chrono::Utc>::from_timestamp(t as i64, 0).unwrap())
        .unwrap_or_else(chrono::Utc::now);
    
    let start = start_time.map(|t| chrono::DateTime::<chrono::Utc>::from_timestamp(t as i64, 0).unwrap())
