...
impl TransactionAnalysisManager {
    ...
    fn get_related_transactions(
        ...
        // 最大10件に制限
        related_txs.sort_by(|a, b| {
            a.time_difference_seconds
                .abs()
                .cmp(&b.time_difference_seconds.abs())
        });
    ...
    fn get_cross_shard_info(
        ...
        if let Some(parent_id) = &transaction.parent_id {
            // 親トランザクションを検索
            let parent_tx = all_transactions.iter().find(|tx| tx.id == *parent_id);
            ...
            if let Some(parent) = parent_tx {
                ...
                let latest_child_time = sibling_txs
                    ...
                    .filter_map(|tx| DateTime::<Utc>::from_timestamp(tx.timestamp as i64, 0))
                    .chain(std::iter::once(
                        DateTime::<Utc>::from_timestamp(transaction.timestamp as i64, 0).unwrap(),
                    ))
                    .max()
                    .unwrap_or(parent_time);

                let completion_time_ms =
                    (latest_child_time - parent_time).num_milliseconds().max(0) as u64;
    ...
    fn assess_risk(
        ...
        if sender_tx_count < 5 && amount > 10000.0 {
            risk_factors.push(RiskFactor {
                ...
                description: "Unusually large transaction for an address with few transactions"
                    .to_string(),
                severity: 60,
            });
