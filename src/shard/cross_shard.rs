...
impl CrossShardTransactionManager {
    ...
    pub fn create_transaction(
        ...
        let destination_shard = self
            .shard_manager
            .get_shard_for_address(&transaction.to_address)?;

        // 同一シャード内のトランザクションの場合はエラー
        if source_shard == destination_shard {
            return Err(Error::InvalidOperation(
                "送信元と送信先が同じシャードです。クロスシャードトランザクションは不要です。"
                    .to_string(),
            ));
        }
    ...
    /// アボート完了通知を処理
    pub fn handle_abort_ack(&self, transaction_id: &str, shard_id: &ShardId) -> Result<(), Error> {
        // トランザクションを取得
        let mut transactions = self.transactions.write().unwrap();
        let tx = transactions.get_mut(transaction_id).ok_or_else(|| {
            Error::NotFound(format!(
...
impl CrossShardTransactionHandler for Shard {
    ...
    /// クロスシャードトランザクションを検証
    fn validate_cross_shard_transaction(
        &self,
    ...
    /// クロスシャードトランザクションを実行
    fn execute_cross_shard_transaction(
        &self,
        transaction: &CrossShardTransaction,
    ) -> Result<(), Error> {
