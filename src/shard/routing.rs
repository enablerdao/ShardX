...
impl RoutingTable {
    ...
    fn dijkstra(
        ...
        // 距離と前のノードを初期化
        let mut distances: HashMap<ShardId, f64> = all_shards
            .iter()
            .map(|shard| (shard.clone(), f64::INFINITY))
            .collect();

        let mut previous: HashMap<ShardId, Option<ShardId>> = all_shards
            .iter()
        ...
        while let Some(State { shard, cost }) = queue.pop() {
            ...
            for neighbor in self.get_neighbors(&shard) {
                ...
                let edge_cost = match criteria {
                    ...
                    OptimizationCriteria::Bandwidth => {
                        1_000_000_000.0 / connection.bandwidth_bps as f64
                    }
    ...
    pub fn optimize(&mut self) {
        // 使用されていない接続を削除
        self.connections
            .retain(|_, conn| conn.enabled && conn.reliability > 0.0);
            ...
            for peer in peers {
                if let Some(peer_shard_id) = shard_manager.get_shard_id_by_peer(&peer) {
                    ...
                    connection.bandwidth_bps =
                        peer.bandwidth_bps.unwrap_or(connection.bandwidth_bps);
