...
impl CrossShardManager {
    ...
    pub fn create_transaction(
        ...
        // 保留中のトランザクションに追加
        self.pending_transactions.insert(cross_shard_tx_id.clone(), cross_shard_tx.clone());
        
        // 送信キューに追加
        self.transmit_queue.push_back(cross_shard_tx);
        
        // メトリクスを更新
        self.metrics.increment_counter("cross_shard_transactions_created");
    ...
    pub fn transmit_transaction(&mut self, transaction_id: &str) -> Result<(), Error> {
        // トランザクションを取得
        let mut tx = self.pending_transactions.get_mut(transaction_id).ok_or_else(|| {
            Error::NotFound(format!("Cross-shard transaction not found: {}", transaction_id))
        })?;
        
        // ステータスをチェック
        if tx.status != CrossShardTransactionStatus::Initialized && tx.status != CrossShardTransactionStatus::SourceCommitted {
        ...
        if tx.status == CrossShardTransactionStatus::Initialized {
            // トランザクションをコミット
            self.consensus_engine.submit_transaction(tx.transaction.clone())?;
            ...
            // メトリクスを更新
            self.metrics.increment_counter("cross_shard_transactions_source_committed");
        }
        ...
        // メトリクスを更新
        self.metrics.increment_counter("cross_shard_transactions_transmitted");
    ...
    pub fn receive_transaction(
        ...
        // 保留中のトランザクションに追加
        self.pending_transactions.insert(transaction_id.to_string(), cross_shard_tx);
        ...
        // トランザクションをコミット
        self.consensus_engine.submit_transaction(transaction.clone())?;
        
        // メトリクスを更新
        self.metrics.increment_counter("cross_shard_transactions_received");
    ...
    pub fn handle_transaction_received(
        ...
        // メトリクスを更新
        self.metrics.increment_counter("cross_shard_transactions_target_received");
    ...
    pub fn handle_transaction_commit(
        ...
        if self.current_shard_id == *source_shard_id {
            ...
            // メトリクスを更新
            self.metrics.increment_counter("cross_shard_transactions_target_committed");
    ...
    pub fn handle_transaction_acknowledge(
        ...
        if self.current_shard_id == *source_shard_id {
            ...
            if tx.confirmations >= tx.required_confirmations {
                ...
                // メトリクスを更新
                self.metrics.increment_counter("cross_shard_transactions_completed");
            }
            
            // メトリクスを更新
            self.metrics.increment_counter("cross_shard_transactions_source_acknowledged");
        } else if self.current_shard_id == *target_shard_id {
            // 送信先シャードの場合
    ...
    pub fn handle_transaction_status_query(
        ...
        let status = if let Some(tx) = self.pending_transactions.get(transaction_id) {
            ...
            return Err(Error::NotFound(format!("Cross-shard transaction not found: {}", transaction_id)));
        };
    ...
    pub fn handle_transaction_status_response(
        ...
        // トランザクションを取得
        let tx = self.pending_transactions.get_mut(transaction_id).ok_or_else(|| {
            Error::NotFound(format!("Cross-shard transaction not found: {}", transaction_id))
        })?;
        ...
        match status {
            ...
            CrossShardTransactionStatus::Failed => {
                ...
                // メトリクスを更新
                self.metrics.increment_counter("cross_shard_transactions_failed");
            },
            _ => {
    ...
    pub fn handle_message(&mut self, message: CrossShardMessage) -> Result<(), Error> {
        match message {
            CrossShardMessage::TransactionTransmit { transaction_id, transaction, source_shard_id, target_shard_id } => {
                self.receive_transaction(&transaction_id, transaction, source_shard_id)?;
            },
            CrossShardMessage::TransactionReceived { transaction_id, source_shard_id, target_shard_id } => {
                self.handle_transaction_received(&transaction_id, &source_shard_id, &target_shard_id)?;
            },
            CrossShardMessage::TransactionCommit { transaction_id, source_shard_id, target_shard_id, status } => {
                self.handle_transaction_commit(&transaction_id, &source_shard_id, &target_shard_id, &status)?;
            },
            CrossShardMessage::TransactionAcknowledge { transaction_id, source_shard_id, target_shard_id } => {
                self.handle_transaction_acknowledge(&transaction_id, &source_shard_id, &target_shard_id)?;
            },
            CrossShardMessage::TransactionStatusQuery { transaction_id, source_shard_id, target_shard_id } => {
                self.handle_transaction_status_query(&transaction_id, &source_shard_id, &target_shard_id)?;
            },
            CrossShardMessage::TransactionStatusResponse { transaction_id, source_shard_id, target_shard_id, status } => {
                self.handle_transaction_status_response(&transaction_id, &source_shard_id, &target_shard_id, &status)?;
            },
        }
            ...
            if tx.status != CrossShardTransactionStatus::Completed &&
               ...
               tx.status != CrossShardTransactionStatus::Cancelled {
                ...
                if elapsed > timeout_duration {
                    ...
                    // メトリクスを更新
                    self.metrics.increment_counter("cross_shard_transactions_timed_out");
                }
            }
        }
        ...
        for id in retry_ids {
            if let Some(tx) = self.pending_transactions.get_mut(&id) {
                ...
                match tx.status {
                    ...
                    CrossShardTransactionStatus::SourceCommitted | CrossShardTransactionStatus::Transmitted => {
                        // 送信キューに追加
                        self.transmit_queue.push_back(tx.clone());
                    },
                ...
                // メトリクスを更新
                self.metrics.increment_counter("cross_shard_transactions_retried");
            }
        }
    ...
    /// メッセージをシャードに送信
    fn send_message_to_shard(&self, shard_id: &ShardId, message: CrossShardMessage) -> Result<(), Error> {
        // シャード情報を取得
        let shard_info = self.shard_manager.get_shard_info(shard_id).ok_or_else(|| {
            Error::NotFound(format!("Shard not found: {}", shard_id))
        })?;
