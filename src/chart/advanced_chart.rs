...
impl OHLCDataSeriesBuilder {
    ...
    /// 出来高付きOHLCデータポイントを追加
    pub fn add_data_point_with_volume(
        mut self,
        timestamp: DateTime<Utc>,
        open: f64,
        high: f64,
    ...
    pub fn add_data_points_with_volume(
        mut self,
        points: Vec<(DateTime<Utc>, f64, f64, f64, f64, f64)>,
    ) -> Self {
...
impl ChartDataAggregator {
    ...
    pub fn aggregate_by_timeframe(
        ...
        while current_frame_start <= end_time {
            ...
            let frame_points: Vec<&DataPoint> = sorted_points
                .iter()
            ...
            if !frame_points.is_empty() {
                // 値の平均を計算
                let avg_value =
                    frame_points.iter().map(|p| p.value).sum::<f64>() / frame_points.len() as f64;
    ...
    /// 時間枠でOHLCデータを集約
    pub fn aggregate_ohlc_by_timeframe(
        ...
        while current_frame_start <= end_time {
            ...
            // 現在の時間枠に含まれるデータポイントを抽出
            let frame_points: Vec<&OHLCDataPoint> = sorted_points
                .iter()
                .filter(|p| p.timestamp >= current_frame_start && p.timestamp < current_frame_end)
                .collect();
            ...
            if !frame_points.is_empty() {
                ...
                // 高値は期間内の最大値
                let high = frame_points
                    .iter()
                    .map(|p| p.high)
                    .fold(f64::NEG_INFINITY, f64::max);

                // 安値は期間内の最小値
                let low = frame_points
                    .iter()
                    .map(|p| p.low)
    ...
    pub fn calculate_moving_average(data_points: &[DataPoint], period: usize) -> Vec<DataPoint> {
        ...
        for i in period - 1..sorted_points.len() {
            let sum: f64 = sorted_points[i - (period - 1)..=i]
                .iter()
                .map(|p| p.value)
                .sum();
    ...
    pub fn calculate_bollinger_bands(
        ...
        for (i, ma_point) in middle_band.iter().enumerate() {
            ...
            if end_idx <= sorted_points.len() {
                ...
                let variance: f64 = sorted_points[start_idx..end_idx]
                    .iter()
    ...
    pub fn calculate_rsi(data_points: &[DataPoint], period: usize) -> Vec<DataPoint> {
        ...
        for i in period..price_changes.len() {
            ...
            if price_changes[i] >= 0.0 {
                current_avg_gain =
                    (current_avg_gain * (period as f64 - 1.0) + price_changes[i]) / period as f64;
                ...
                current_avg_loss =
                    (current_avg_loss * (period as f64 - 1.0) - price_changes[i]) / period as f64;
            }

            // RSIを計算
            rs = if current_avg_loss == 0.0 {
                100.0
            } else {
    ...
    pub fn calculate_macd(
        ...
        for i in 0..signal_line.len() {
            let hist_value =
                macd_line[i + macd_line.len() - signal_line.len()].value - signal_line[i].value;
...
impl ChartRenderer for HTMLChartRenderer {
    fn render_chart(&self, chart: &Chart) -> Result<String, Error> {
        ...
        let html = format!(
            ...
            width,
            height,
            chart.config.chart_type.to_string().to_lowercase()
        );
...
mod tests {
    ...
    fn test_chart_data_aggregator() {
        ...
        // 30分間隔で集約
        let aggregated =
            ChartDataAggregator::aggregate_by_timeframe(&data_points, &TimeFrame::Minute(30));
