...
use crate::identity::did::{DIDDocument, DID};
use crate::identity::schema::{Schema, SchemaValidator};
use crate::identity::verifiable_credential::{
    CredentialProof, CredentialSchema, CredentialStatus, CredentialSubject,
    Issuer as CredentialIssuer, VerifiableCredential,
};
...
impl BasicIssuer {
    ...
    fn sign_credential(&self, credential: &mut VerifiableCredential) -> Result<(), Error> {
        ...
        let credential_json = serde_json::to_string(credential).map_err(|e| {
            Error::SerializationError(format!("Failed to serialize credential: {}", e))
        })?;

        // 署名を作成
        let signature = self
            .key_pair
            .sign(credential_json.as_bytes())
...
impl Issuer for BasicIssuer {
    ...
    fn issue_credential(
        ...
        if options.schema_validation.unwrap_or(true) {
            if let Some(schema) = &schema {
                ...
                let subject_json = serde_json::to_value(&subject).map_err(|e| {
                    Error::SerializationError(format!("Failed to serialize subject: {}", e))
                })?;

                if !self.schema_validator.validate(schema_id, &subject_json)? {
                    return Err(Error::ValidationError(
                        "Subject does not match schema".to_string(),
                    ));
                }
            }
        }
        ...
        // クレデンシャルを作成
        let mut credential =
            VerifiableCredential::new(types, issuer, subject).with_id(credential_id.clone());
    ...
    fn verify_credential(&self, credential: &VerifiableCredential) -> Result<bool, Error> {
        ...
        let issuer_did = match &credential.issuer {
            CredentialIssuer::Uri(uri) => DID::parse(uri)?,
            CredentialIssuer::Object(obj) => DID::parse(&obj.id)?,
        };
        ...
        if let Some(proof) = &credential.proof {
            ...
            // クレデンシャルをシリアライズ
            let credential_json =
                serde_json::to_string(&credential_without_proof).map_err(|e| {
                    Error::SerializationError(format!("Failed to serialize credential: {}", e))
                })?;
            ...
            let valid = self
                .key_pair
                .public_key()
                .verify(credential_json.as_bytes(), &signature)
    ...
    fn update_credential(
        ...
        issuer
            .credentials
            .insert(credential_id.clone(), new_credential.clone());
