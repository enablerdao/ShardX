...
impl MultisigTransaction {
    ...
    pub fn add_signature(&mut self, signer_id: &str, signature: Vec<u8>) -> Result<(), String> {
        if !self.signatures.contains_key(signer_id) {
            return Err(format!(
                "Signer {} is not authorized for this transaction",
                signer_id
            ));
        }
...
impl MultisigManager {
    ...
    pub fn create_transaction(
        ...
        if !wallet.signers.contains(&creator_id.to_string()) {
            return Err(format!(
                "Creator {} is not a signer of wallet {}",
                creator_id, wallet_id
            ));
        }
    ...
    pub fn get_transactions_by_account(&self, account_id: &str) -> Vec<MultisigTransaction> {
        ...
        transactions
            .values()
            .filter(|tx| {
                wallet_ids.contains(&tx.wallet_id) && tx.signatures.contains_key(account_id)
            })
            .cloned()
            .collect()
    }
    ...
    pub fn sign_transaction(
        ...
        if !wallet.signers.contains(&signer_id.to_string()) {
            // セキュリティログを記録
            warn!(
                "Unauthorized signature attempt: signer={}, tx={}",
                signer_id, tx_id
            );
            return Err(format!(
                "Signer {} is not authorized for this transaction",
        ...
        // 署名イベントを記録
        info!(
            "Transaction signed: tx={}, signer={}, wallet={}",
            tx_id, signer_id, wallet.id
        );
    ...
    fn verify_signature(
        ...
        if signature.len() < 32 {
            warn!(
                "Invalid signature format: signer={}, tx={}",
                signer_id, tx.id
            );
    ...
    fn process_transaction(&self, tx: &mut MultisigTransaction) -> Result<(), String> {
        ...
        match tx_data.operation {
            ...
            Operation::Transfer {
                ...
                info!(
                    "Transfer executed: {} from wallet {} to {}",
                    amount, tx.wallet_id, to
                );
            }
            ...
            Operation::ChangeRequiredSignatures { required } => {
                ...
                info!(
                    "Required signatures changed to {} for wallet {}",
                    required, tx.wallet_id
                );
            }
        }
...
mod tests {
    ...
    fn test_multisig_wallet_validation() {
        // 署名者が空の場合
        let wallet =
            MultisigWallet::new("Test Wallet".to_string(), "owner1".to_string(), vec![], 1);
