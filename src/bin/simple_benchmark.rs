...
async fn main() -> Result<(), Error> {
    ...
    for chunk in transactions.chunks(batch_size) {
        ...
        for tx in chunk {
            ...
            let handle = tokio::spawn(async move {
                process_transaction(&tx, &shard_manager).await
            });
        ...
        info!("Batch processed: {} transactions in {:.2}s ({:.2} TPS)",
            batch_size, batch_time.as_secs_f64(), tps);
    }
    ...
    if total_tps >= 100000.0 {
        ...
        info!("❌ FAILED: Did not achieve 100K TPS. Reached {:.2} TPS", total_tps);
        
        // 理論上の最大TPSを計算
        let theoretical_max = 1000000.0 / 0.01; // 10マイクロ秒あたり1トランザクション
        info!("Theoretical maximum on this hardware: {:.2} TPS", theoretical_max);
...
/// テストトランザクションを生成
async fn generate_test_transactions(count: usize, shard_manager: &ShardManager) -> Result<Vec<Transaction>, Error> {
    // アクティブなシャードを取得
