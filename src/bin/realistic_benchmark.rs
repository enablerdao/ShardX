...
impl Shard {
    ...
    fn process_transaction(
        &mut self,
        tx: &Transaction,
        network: &NetworkLatencySimulator,
        disk: &DiskIOSimulator,
    ) -> bool {
        // ネットワーク遅延をシミュレート
    ...
    fn create_block(
        ...
        let max_txs = 1000;
        let txs_to_include: Vec<Transaction> = self
            .pending_txs
            .drain(..std::cmp::min(max_txs, self.pending_txs.len()))
            .collect();
...
fn run_benchmark(config: &BenchmarkConfig) -> BenchmarkResult {
    ...
    let tx_thread = thread::spawn(move || {
        ...
        for _ in 0..config.num_transactions {
            ...
            let amount =
                rng.gen_range((sender_balance / 100).max(1)..=(sender_balance / 10).max(1));
...
fn main() {
    ...
    println!(
        "Average transaction latency: {:.2} ms",
        result.average_transaction_latency_ms
    );
    println!(
    ...
    println!(
        "High Load: {:.2} TPS",
        high_load_result.transactions_per_second
    );
    println!("Optimal: {:.2} TPS", optimal_result.transactions_per_second);
    println!(
        "Multi-Shard: {:.2} TPS",
        multi_shard_result.transactions_per_second
    );
}
